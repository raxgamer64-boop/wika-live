...
const int zegoAppId = 1236782539;
...
class _LiveScreenState extends State<LiveScreen> {
  ...
  Widget _liveCard(BuildContext context, List<String> host) {
    return InkWell(
      onTap: () => openPage(context, LiveRoomPage(host: host[0], viewers: '${host[1]} viewers', liveId: 'demo_${host[0]}', hostUid: 'demo_${host[0]}')),
      borderRadius: BorderRadius.circular(20),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(20),
        child: Stack(
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (_, i) {
          ...
          return InkWell(
            onTap: () => openPage(context, LiveRoomPage(host: h[0], viewers: '${h[1]} viewers', liveId: 'demo_${h[0]}', hostUid: 'demo_${h[0]}')),
...
class MeScreen extends StatelessWidget {
  ...
  Widget _toolPage(String label) {
    switch (label) {
      ...
      case 'Game': return const FeaturePage(title: 'Live Features', icon: '🎮', items: ['Mini games', 'Game ranking', 'Game rewards']);
...
class _GoLivePageState extends State<GoLivePage> {
  final title = TextEditingController(text: 'WikaLive');
  ...
  Future<void> startLive() async {
    ...
    final liveTitle =
        title.text.trim().isEmpty ? 'WikaLive' : title.text.trim();
    final liveId = 'live_${user.uid}';
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: const LinearGradient(
                colors: [Color(0xFF7B2CBF), Color(0xFFE052A5)],
              ),
            ),
            child: const Center(
              child: Icon(
          ...
          const SizedBox(height: 8),
          const Text(
            'Camera and microphone will be used in the live room.',
...
class _LiveRoomPageState extends State<LiveRoomPage> {
  ...
  Future<void> _endHostLive() async {
    if (!widget.isHost) return;

    await FirebaseFirestore.instance.collection('users').doc(widget.hostUid).set(
      {
        'isLive': false,
      ...
      return Scaffold(
        ...
        body: const Center(
          child: Padding(
            ...
            child: Text(
              'ZEGOCLOUD AppSign is missing from this build.',
              textAlign: TextAlign.center,
