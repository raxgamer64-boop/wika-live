...
class _GuestCallOverlay extends StatelessWidget {
  final bool isHost;
  final String hostName;
  ...
  @override
  Widget build(BuildContext context) {
    final displayName = hostName.trim().isEmpty ? 'WikaLive Host' : hostName.trim();
    return Stack(
      children: [
        Positioned(
          left: 12,
            ...
            child: Row(
              children: [
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6),
                  decoration: BoxDecoration(
                    color: const Color(0xAA0B0A12),
                    borderRadius: BorderRadius.circular(22),
                    border: Border.all(color: const Color(0x665F4A77)),
                  ),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      const CircleAvatar(
                        radius: 17,
                        backgroundColor: Color(0xFF56306D),
                      ...
                      Container(
                        ...
                        child: Text(isHost ? 'LIVE' : 'FOLLOW', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900)),
                      ),
                    ],
                  ),
                ),
                ...
                Container(
                  ...
                  child: const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 5),
                      Text('1.2K', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 11)),
                    ],
                  ),
                ),
              ],
          ...
          child: SafeArea(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: const Color(0xAA0B0A12),
                borderRadius: BorderRadius.circular(14),
                border: Border.all(color: const Color(0x553F3152)),
              ),
              child: const Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.local_fire_department_rounded, color: Color(0xFFFFB54A), size: 15),
                  SizedBox(width: 4),
                  Text('Top 1', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800)),
                  SizedBox(width: 8),
                  Icon(Icons.auto_awesome_rounded, color: Color(0xFFFF5DC8), size: 14),
                  SizedBox(width: 4),
                  Text('5.0K', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800)),
                ],
              ),
            ),
          ),
        ...
        Positioned(
          ...
          child: SafeArea(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                _guestSlot(context, 0),
                const SizedBox(height: 7),
                _guestSlot(context, 1),
                const SizedBox(height: 7),
                _guestSlot(context, 2, plus: true),
              ],
            ),
          ),
        ),
          ...
          child: SafeArea(
            child: IgnorePointer(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: const [
                  _LiveChatBubble(name: 'Rohit', text: 'Hello ❤️', icon: Icons.favorite_rounded),
                  SizedBox(height: 5),
                  _LiveChatBubble(name: 'Sameer', text: 'Amazing live!', icon: Icons.auto_awesome_rounded),
                  SizedBox(height: 5),
                  _LiveChatBubble(name: 'Neha', text: 'Sent you a gift 🎁', icon: Icons.card_giftcard_rounded),
                ],
              ),
            ),
          ),
        ...
        Positioned(
          ...
          child: SafeArea(
            child: InkWell(
              onTap: () => msg(context, 'Gift panel is ready for wallet/gift integration.'),
              borderRadius: BorderRadius.circular(18),
              child: Container(
                width: 62,
                height: 62,
                decoration: BoxDecoration(
                  gradient: const LinearGradient(colors: [Color(0xFFFF39C8), Color(0xFF7B35FF)]),
                  borderRadius: BorderRadius.circular(18),
                  border: Border.all(color: const Color(0xFFFFA4F0), width: 1.2),
                  boxShadow: const [BoxShadow(color: Color(0x663F0D55), blurRadius: 18)],
                ),
                child: const Icon(Icons.card_giftcard_rounded, color: Colors.white, size: 29),
              ),
            ),
          ),
        ),
  ...
  Widget _guestSlot(BuildContext context, int index, {bool plus = false}) {
    return StreamBuilder<List<ZegoUIKitUser>>(
      stream: ZegoUIKitPrebuiltLiveStreamingController().user.stream(includeFakeUser: false),
      builder: (context, snapshot) {
        final users = (snapshot.data ?? <ZegoUIKitUser>[]).where((u) => u.id != FirebaseAuth.instance.currentUser?.uid).toList();
        final guests = users.where((u) => u.camera.value || u.microphone.value).toList();
        final guest = index < guests.length ? guests[index] : null;
        if (plus && guest == null) {
          return InkWell(
            borderRadius: BorderRadius.circular(15),
            ...
            child: Container(
              width: 62,
              height: 62,
              decoration: BoxDecoration(color: const Color(0xAA15121C), borderRadius: BorderRadius.circular(15), border: Border.all(color: const Color(0xFFE05CFF), width: 1.5)),
              child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.add_rounded, color: Colors.white, size: 27), Text('Guest', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800))]),
            ),
          );
        }
        if (guest == null) return const SizedBox(width: 62, height: 62);
        return Container(
          width: 62,
          height: 62,
          decoration: BoxDecoration(color: const Color(0xAA15121C), borderRadius: BorderRadius.circular(15), border: Border.all(color: const Color(0xFFB86DFF))),
          child: Center(child: Text(guest.name.isNotEmpty ? guest.name.substring(0, 1).toUpperCase() : '?', style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900))),
        );
      },
    );
  }
            ...
            builder: (context, snapshot) {
              ...
              return Padding(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const SizedBox(height: 12),
                  if (users.isEmpty)
                    const Padding(padding: EdgeInsets.symmetric(vertical: 28), child: Center(child: Text('No viewers available yet.', style: TextStyle(color: Color(0xFF9C95A8)))))
                  else
                    Flexible(child: ListView.separated(shrinkWrap: true, itemCount: users.length, separatorBuilder: (_, __) => const Divider(color: Color(0xFF282332)), itemBuilder: (_, i) {
                      final u = users[i];
                      return ListTile(
                        ...
                        trailing: FilledButton(
                          onPressed: () async {
                            if (isHost) {
                              final ok = await controller.coHost.hostSendCoHostInvitationToAudience(u, withToast: true);
                              if (sheetContext.mounted) Navigator.of(sheetContext).pop();
                              if (!ok && context.mounted) msg(context, 'Guest invitation could not be sent.');
                            } else {
                              final ok = await controller.coHost.audienceSendCoHostRequest(withToast: true);
                              if (sheetContext.mounted) Navigator.of(sheetContext).pop();
                              if (!ok && context.mounted) msg(context, 'Join request could not be sent.');
                            }
                          },
                          child: Text(isHost ? 'Invite' : 'Join'),
                        ),
                      );
                    })),
                ]),
            ...
            child: Column(
              ...
              children: [
                ...
                _toolSection('Room tools', [
                  ...
                  _sheetTool(Icons.face_retouching_natural_rounded, 'Beauty', () => msg(sheetContext, 'Beauty controls are available from the live toolbar.')),
                ...
                _toolSection('Basic tools', [
                  ...
                  _sheetTool(Icons.mic_none_rounded, 'Microphone', () => controller.microphone.toggle()),
                  _sheetTool(Icons.videocam_outlined, 'Camera', () => controller.camera.toggle()),
                  _sheetTool(Icons.flip_camera_android_rounded, 'Mirror', () => msg(sheetContext, 'Mirror setting ready.')),
  ...
  Widget _toolSection(String title, List<Widget> items) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(title, style: const TextStyle(color: Color(0xFFE6E0EF), fontSize: 13, fontWeight: FontWeight.w900)),
      const SizedBox(height: 9),
      Wrap(spacing: 8, runSpacing: 8, children: items),
    ]);
  }

  Widget _sheetTool(IconData icon, String label, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(15),
      child: Container(
        width: 82,
        height: 78,
        decoration: BoxDecoration(color: const Color(0xFF17141F), borderRadius: BorderRadius.circular(15), border: Border.all(color: const Color(0xFF30263B))),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(icon, color: const Color(0xFFE06AFF), size: 24),
          const SizedBox(height: 6),
          Text(label, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800)),
        ]),
      ),
    );
  }
...
class _LiveChatBubble extends StatelessWidget {
  ...
  final String text;
  final IconData icon;
  const _LiveChatBubble({required this.name, required this.text, required this.icon});

  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
      decoration: BoxDecoration(color: const Color(0xAA111019), borderRadius: BorderRadius.circular(15), border: Border.all(color: const Color(0x442D2439))),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Text('$name: ', style: const TextStyle(color: Color(0xFFFFC2F1), fontSize: 10, fontWeight: FontWeight.w900)),
        Flexible(child: Text(text, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700))),
        const SizedBox(width: 6),
        Icon(icon, color: Color(0xFFFF5ACB), size: 13),
      ]),
    );
  }
}
