...
const int kMaxSvipLevel = 18;

String svipFrameAsset(int level) {
  final safe = level.clamp(1, kMaxSvipLevel);
  return 'assets/svip/SVIP_$safe.png';
}

int readSvipLevel(Map<String, dynamic> data) {
  final value = data['svipLevel'];
  if (value is num) return value.toInt().clamp(0, kMaxSvipLevel);
  return 0;
}

int readEquippedSvipLevel(Map<String, dynamic> data) {
  final value = data['equippedSvipFrame'];
  if (value is num) return value.toInt().clamp(0, kMaxSvipLevel);
  return 0;
}

class SvipAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          // The profile picture is ALWAYS a true circle.
          profile,

          // The SVIP PNG is a separate transparent layer. It is never
          // clipped, so the crown, side ornaments and bottom badge remain
          // fully visible outside the circular photo.
          if (showFrame && svipLevel > 0)
            Positioned.fill(
              child: IgnorePointer(
                child: Image.asset(
                  svipFrameAsset(svipLevel),
                  width: size,
                  height: size,
                  fit: BoxFit.contain,
                  alignment: Alignment.center,
...
class SvipBackpackPage extends StatelessWidget {
  const SvipBackpackPage({super.key});

  Future<void> _use(BuildContext context, int level) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null || level < 1) return;
    await FirebaseFirestore.instance.collection('users').doc(user.uid).set({
      'equippedSvipFrame': level,
      'equippedSvipFrameUpdatedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));
    if (context.mounted) msg(context, 'SVIP $level frame equipped');
  }
  ...
  Future<void> _remove(BuildContext context) async {
    ...
    if (user == null) return;
    await FirebaseFirestore.instance.collection('users').doc(user.uid).set({
      'equippedSvipFrame': FieldValue.delete(),
      'equippedSvipFrameUpdatedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));
    if (context.mounted) msg(context, 'SVIP frame removed');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Backpack', style: TextStyle(fontWeight: FontWeight.w900)),
      ),
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance.collection('users').doc(user.uid).snapshots(),
        builder: (context, snapshot) {
          final data = snapshot.data?.data() ?? <String, dynamic>{};
          final level = readSvipLevel(data);
          final equipped = data['equippedSvipFrame'] is num
              ? (data['equippedSvipFrame'] as num).toInt()
              : 0;
          final photo = (data['photoUrl'] ?? user.photoURL ?? '').toString();

          if (level < 1) {
            return const Center(
              child: Text('No SVIP frame unlocked yet.', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w700)),
            );
          }
          ...
          return ListView(
            ...
            children: [
              Container(
                ...
                child: Column(
                  children: [
                    const Text('CURRENT SVIP FRAME', style: TextStyle(color: Colors.white70, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.2)),
                    const SizedBox(height: 8),
                    Text('SVIP $level', style: const TextStyle(color: Colors.white, fontSize: 25, fontWeight: FontWeight.w900)),
                    const SizedBox(height: 18),
                    SvipAvatar(photoUrl: photo, svipLevel: level, size: 150),
                    const SizedBox(height: 12),
                    Text(equipped == level ? 'Currently in use' : 'Unlocked frame', style: const TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w700)),
                    const SizedBox(height: 16),
                    Row(
                      children: [
                        Expanded(
                          child: FilledButton(
                            onPressed: equipped == level ? null : () => _use(context, level),
                            child: Text(equipped == level ? 'Used' : 'Use'),
                          ),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: OutlinedButton(
                            onPressed: equipped == 0 ? null : () => _remove(context),
            ...
            child: Stack(
              ...
              children: [
                photo.isEmpty
                    ...
                    : Image(
                        image: profileImageProvider(photo),
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => Container(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          child: ListTile(
            leading: CircleAvatar(
              backgroundImage:
                  photo.isNotEmpty ? profileImageProvider(photo) : null,
              child: photo.isEmpty
...
class MeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        final svipLevel = readSvipLevel(d);
        final equippedSvipLevel = readEquippedSvipLevel(d);
          ...
          Row(
            ...
            children: [
              Stack(
                clipBehavior: Clip.none,
                children: [
                  SvipAvatar(photoUrl: photo, svipLevel: svipLevel, size: 92),
                  Positioned(
                    right: 0,
                    bottom: 0,
                    child: InkWell(
  ...
  Widget _mySpaceCard(BuildContext context) {
    final tools = <_MeTool>[
      ...
      _MeTool(Icons.backpack_rounded, 'Backpack', const SvipBackpackPage()),
              ...
              SizedBox(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (_, i) => ClipRRect(
                    ...
                    child: Image(
                      image: profileImageProvider(photos[i]),
                      width: 42,
                      height: 48,
                      fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) => Container(
...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
  ...
  Widget _avatar({String? url, String? name, double radius = 22}) {
    ...
    if (clean.isNotEmpty) {
      return CircleAvatar(radius: radius, backgroundImage: profileImageProvider(clean));
    }
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<ZegoUIKitUser>>(
      ...
      builder: (context, roomSnapshot) {
        ...
        final audienceCount =
            roomUsers.where((u) => u.id != selfId).length;
        final guests = roomUsers
            .where((u) =>
                u.id != selfId && (u.camera.value || u.microphone.value))
            .take(3)
            .toList();
        ...
        return Stack(
          fit: StackFit.expand,
          children: [
            // Top row: host, guardian, supporter avatars, viewers, close.
            Positioned(
              top: top + 14,
              left: 14,
              right: 14,
              child: _compactTopHeader(audienceCount),
            ),

            // Second icon row from the reference layout.
            Positioned(
              top: top + 82,
              left: 14,
            ...
            // Guest/co-host slot stays on the right over the live video.
            Positioned(
              right: 14,
              top: media.size.height * .43,
                ...
                builder: (context, messageSnapshot) {
                  ...
                  return ConstrainedBox(
                    constraints: BoxConstraints(
                      maxHeight: media.size.height * .28,
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      mainAxisSize: MainAxisSize.min,
            ...
            Positioned(
              left: 16,
              bottom: bottom + 82,
              right: media.size.width * .58,
              child: _warningText(),
            ),

            // Floating hearts stay on the right edge.
            Positioned(
              right: 12,
              top: top + 430,
              child: IgnorePointer(
            ...
            // Chat input stays on the lower-left like the reference.
            Positioned(
              left: 16,
              bottom: bottom + 12,
              child: SizedBox(
            ...
            // Reference bottom controls: More, Beauty, Mic, Gift.
            Positioned(
              right: 12,
              bottom: bottom + 10,
              child: _referenceBottomControls(),
            ),
          ],
        );
      },
  ...
  Widget _referenceBottomControls() {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        _bottomButton(
  ...
  Widget _skyRankingCard() {
    return _glass(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: const [
          Icon(Icons.star_rounded, color: Color(0xFFFFD34D), size: 20),
          SizedBox(width: 6),
          Text(
            'Sky Ranking',
            style: TextStyle(
  ...
  Widget _announcementBanners() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        _announcement(
          icon: Icons.emoji_events_rounded,
          text: '2215times  Super Win',
          accent: const Color(0xFFFFD83D),
        ),
        const SizedBox(height: 7),
        _announcement(
        ...
        _announcement(
          ...
          text: 'Oily sent a gift to Flora...',
  ...
  Widget _announcement({
    ...
    return ConstrainedBox(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: const Color(0xD91B1510),
          borderRadius: BorderRadius.circular(28),
          border: Border.all(color: accent.withOpacity(.72)),
          boxShadow: [
            BoxShadow(
            ...
            Expanded(
              child: Text(
                ...
                style: TextStyle(
                  ...
                  fontWeight: FontWeight.w900,
                  shadows: [
                    Shadow(color: accent.withOpacity(.55), blurRadius: 8),
                  ],
                ),
              ),
            ),
  ...
  Widget _walletBadge() {
    ...
    if (uid == null) return _walletPill(0);
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: _userStream(uid),
      builder: (context, snapshot) {
        final data = snapshot.data?.data() ?? <String, dynamic>{};
        final value = data['liveDiamonds'] is num
            ? (data['liveDiamonds'] as num).toInt()
            : 0;
        return _walletPill(value);
      },
    );
  }

  Widget _walletPill(int value) {
    return _glass(
  ...
  Widget _warningText() {
    return const Text(
      'Please keep the live room respectful. Content that violates community rules may be removed.',
      maxLines: 3,
      overflow: TextOverflow.ellipsis,
      style: TextStyle(
  ...
  Widget _bottomGiftWithTimer(VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: const LinearGradient(
                ...
                colors: [Color(0xFFFFD45C), Color(0xFFE88A24)],
              ),
              border: Border.all(color: Colors.white.withOpacity(.65)),
              boxShadow: const [
                BoxShadow(
                  color: Color(0x99FFB52E),
                  blurRadius: 20,
                  spreadRadius: 2,
                ),
              ],
  ...
  Widget _compactTopHeader(int audienceCount) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final closeSize = narrow ? 48.0 : 54.0;
        final viewerWidth = narrow ? 66.0 : 78.0;
        final guardianSize = narrow ? 58.0 : 66.0;
        final hostWidth = narrow ? 218.0 : 242.0;
  ...
  Widget _hostCard() {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final svipLevel = readEquippedSvipLevel(data);
        final photo = _photo(data, widget.hostPhotoUrl);
        return _hostIdentity(name, photo, level, svipLevel);
      },
    );
  }

  Widget _hostIdentity(String name, String photo, int level, int svipLevel) {
    return _glass(
      ...
      child: Row(
        ...
        children: [
          Stack(
            clipBehavior: Clip.none,
            children: [
              SvipAvatar(photoUrl: photo, svipLevel: svipLevel, size: 54),
              Positioned(
                right: -1,
                bottom: -1,
                child: Container(
        ...
        return SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final amount = d['diamondsSent'] is num
                  ? (d['diamondsSent'] as num).toInt()
                  : 0;
              return _rankDataAvatar(
                index + 1,
                name,
                photo,
                amount,
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: const Color(0xFFDDA2FF),
                width: 1.7,
              ),
              boxShadow: const [
                BoxShadow(
                  color: Color(0xCC9B3CFF),
                  blurRadius: 18,
                  spreadRadius: 2,
                ),
              ],
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          final level = d['level'] is num ? (d['level'] as num).toInt() : 1;
          final svipLevel = readEquippedSvipLevel(d);
          ...
          return ListView(
            ...
            children: [
              Container(
                ...
                child: Row(children: [
                  Stack(children: [
                    SvipAvatar(photoUrl: photoUrl, svipLevel: svipLevel, size: 90),
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _addPhoto() async {
    ...
    try {
      ...
      // Store small compressed profile images directly in Firestore so
      // Firebase Storage/Billing is not required for profile photos.
      if (bytes.length > 100000) {
        if (mounted) msg(context, 'Photo is too large. Please choose a smaller photo.');
        return;
      }

      final photoData = 'data:image/jpeg;base64,${base64Encode(bytes)}';
      final updatedPhotos = [...photos, photoData];
      await FirebaseFirestore.instance.collection('users').doc(u.uid).set({
        'photos': updatedPhotos,
        'photoUrl': updatedPhotos.first,
        'updatedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Column(children: [
                  ...
                  const SizedBox(height: 6),
                  CircleAvatar(radius: i == 1 ? 48 : 39, backgroundImage: url.isNotEmpty ? profileImageProvider(url) : null, child: url.isEmpty ? const Icon(Icons.person, size: 35) : null),
                  const SizedBox(height: 7),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          CircleAvatar(radius: 30, backgroundImage: url.isNotEmpty ? profileImageProvider(url) : null, child: url.isEmpty ? const Icon(Icons.person) : null),
                ...
                child: photos.isEmpty
                    ...
                    : PageView.builder(
                        ...
                        itemBuilder: (context, index) => Image(
                          image: profileImageProvider(photos[index]),
                          fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) => Container(
                            color: soft,
                            child: const Icon(Icons.person, size: 110),
