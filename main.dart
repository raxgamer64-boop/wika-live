...
class MeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final followingStream = userRef.collection('following').snapshots();
    final followersStream = userRef.collection('followers').snapshots();
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        final bio = (data['bio'] ?? 'Welcome to WikaLive').toString();
        ...
        return Container(
          ...
          child: SafeArea(
            child: ListView(
              ...
              children: [
                Row(children: [
                  const Expanded(child: Text('My Space', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w900, letterSpacing: -0.5))),
                ...
                const SizedBox(height: 14),
                InkWell(
                  onTap: () => openPage(context, ProfilePage(userId: uid, isOwnProfile: true)),
                  borderRadius: BorderRadius.circular(28),
                  child: _profileHero(name, shortId, bio, country),
                ),
                const SizedBox(height: 14),
                  ...
                  builder: (context, followingSnap) {
                    ...
                    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                      ...
                      builder: (context, followersSnap) {
                        ...
                        return _statsCard(context, following, followers);
                      },
                    );
                  },
                ),
                ...
                const SizedBox(height: 8),
                const Center(child: Text('WikaLive  •  Your space, your story', style: TextStyle(color: Color(0xFFAAA6B4), fontSize: 11, fontWeight: FontWeight.w600))),
              ],
            ),
          ),
        );
  ...
  Widget _topIcon(IconData icon, {VoidCallback? onTap}) => InkWell(
    onTap: onTap,
    borderRadius: BorderRadius.circular(15),
    child: Container(width: 44, height: 44, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(15), boxShadow: const [BoxShadow(color: Color(0x12000000), blurRadius: 14, offset: Offset(0, 5))]), child: Icon(icon, color: const Color(0xFF24212B), size: 23)),
  );

  Widget _profileHero(String name, String id, String bio, String country) => Container(
    padding: const EdgeInsets.all(18),
    decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF2A1C43), Color(0xFF6B3D91), Color(0xFFB34D9B)]), borderRadius: BorderRadius.circular(28), boxShadow: const [BoxShadow(color: Color(0x301D1030), blurRadius: 24, offset: Offset(0, 12))]),
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 5),
        Text(bio, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white70, fontSize: 11)),
        const SizedBox(height: 7),
        _miniBadge('🌍', country),
      ])),
  ...
  Widget _miniBadge(String icon, String text) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: const Color(0x22FFFFFF), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0x22FFFFFF))), child: Text('$icon $text', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800)));

  Widget _statsCard(BuildContext context, int following, int followers) => Container(
    ...
    child: Row(children: [
      _statButton(context, '0', 'Friends', const SimplePage(title: 'Friends', items: ['Your friends will appear here.'])),
      _divider(),
      _statButton(context, '$following', 'Following', FollowingPage(userId: FirebaseAuth.instance.currentUser!.uid)),
      _divider(),
      _statButton(context, '$followers', 'Followers', FollowersPage(userId: FirebaseAuth.instance.currentUser!.uid)),
      _divider(),
      _statButton(context, '0', 'Visitors', const SimplePage(title: 'Visitors', items: ['Visitor history will appear here.'])),
    ]),
  );

  Widget _statButton(BuildContext context, String value, String label, Widget page) => Expanded(child: InkWell(onTap: () => openPage(context, page), child: Column(children: [Text(value, style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w900, color: Color(0xFF22202A))), const SizedBox(height: 4), Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: Color(0xFF9994A3)))])));

  Widget _divider() => Container(width: 1, height: 34, color: const Color(0xFFEDEAF1));

  Widget _kingCard(BuildContext context) => InkWell(onTap: () => openPage(context, const FeaturePage(title: 'King of Kings', icon: '💎', items: ['Premium status', 'Exclusive privileges', 'Special rewards'])), borderRadius: BorderRadius.circular(25), child: Container(height: 126, padding: const EdgeInsets.symmetric(horizontal: 18), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF17111E), Color(0xFF4B2267), Color(0xFF9A4B82)]), borderRadius: BorderRadius.circular(25), boxShadow: const [BoxShadow(color: Color(0x281B1025), blurRadius: 18, offset: Offset(0, 8))]), child: Row(children: [Container(width: 58, height: 58, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0x25FFFFFF), border: Border.all(color: const Color(0x55FFFFFF))), child: const Center(child: Text('💎', style: TextStyle(fontSize: 32)))), const SizedBox(width: 13), const Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('KING OF KINGS', style: TextStyle(color: Color(0xFFFFD98B), fontSize: 12, letterSpacing: 1.2, fontWeight: FontWeight.w900)), SizedBox(height: 5), Text('Unlock your royal privileges', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('Exclusive status • special benefits', style: TextStyle(color: Color(0xFFDCCFE3), fontSize: 10.5, fontWeight: FontWeight.w600))])), const SizedBox(width: 8), Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 11), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFFFD66B), Color(0xFFFF9D36)]), borderRadius: BorderRadius.circular(16)), child: const Text('ACTIVATE', style: TextStyle(color: Color(0xFF3A2411), fontSize: 11, fontWeight: FontWeight.w900)))])));

  Widget _coinCard(BuildContext context, String coins) => InkWell(onTap: () => openPage(context, const WalletPage()), borderRadius: BorderRadius.circular(22), child: Container(height: 112, padding: const EdgeInsets.all(15), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFFFF3B9), Color(0xFFFFE7A1)]), borderRadius: BorderRadius.circular(22)), child: Row(children: [Container(width: 48, height: 48, decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0xFFFFD66B)), child: const Center(child: Text('🪙', style: TextStyle(fontSize: 28)))), const SizedBox(width: 10), Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFFFC83D), Color(0xFFFF7638)]), borderRadius: BorderRadius.circular(12)), child: const Text('RECHARGE', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900))), const SizedBox(height: 7), Text(coins, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900))]))])));

  Widget _gemCard(BuildContext context, String diamonds) => InkWell(onTap: () => openPage(context, const DiamondWithdrawalPage()), borderRadius: BorderRadius.circular(22), child: Container(height: 112, padding: const EdgeInsets.all(15), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFF0E0FF), Color(0xFFE5D1FF)]), borderRadius: BorderRadius.circular(22)), child: Row(children: [Container(width: 48, height: 48, decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0x33FFFFFF)), child: const Center(child: Text('💎', style: TextStyle(fontSize: 30)))), const SizedBox(width: 10), Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('DIAMONDS', style: TextStyle(fontSize: 11, letterSpacing: 1, color: Color(0xFF777083), fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(diamonds, style: const TextStyle(fontSize: 23, fontWeight: FontWeight.w900))])])));

  Widget _inviteCard(BuildContext context) => InkWell(onTap: () => openPage(context, const FeaturePage(title: 'Invite & Earn', icon: '🎁', items: ['Invite users', 'Referral rewards', 'Invitation history'])), borderRadius: BorderRadius.circular(24), child: Container(height: 122, padding: const EdgeInsets.symmetric(horizontal: 20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF3A1A52), Color(0xFF8B3F7A), Color(0xFFD36B75)]), borderRadius: BorderRadius.circular(24), boxShadow: const [BoxShadow(color: Color(0x221F1028), blurRadius: 16, offset: Offset(0, 7))]), child: Row(children: [Container(width: 52, height: 52, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0x22FFFFFF), border: Border.all(color: const Color(0x55FFFFFF))), child: const Icon(Icons.card_giftcard_rounded, color: Color(0xFFFFD16B), size: 28)), const SizedBox(width: 14), const Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('INVITE & EARN', style: TextStyle(color: Color(0xFFFFD16B), fontSize: 11, letterSpacing: 1.1, fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('Earn up to ₹2,250', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)), SizedBox(height: 3), Text('Invite new users and unlock rewards', style: TextStyle(color: Color(0xFFE8DCEB), fontSize: 10.5, fontWeight: FontWeight.w600))])), const Icon(Icons.arrow_forward_ios_rounded, color: Colors.white70, size: 17)])));
  ...
  Widget _sectionTitle(String title, String count) => Row(children: [Expanded(child: Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: Color(0xFF292630)))), Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: const Color(0xFFEDE7F6), borderRadius: BorderRadius.circular(10)), child: Text(count, style: const TextStyle(color: Color(0xFF73538E), fontSize: 9.5, fontWeight: FontWeight.w800)))]);

  Widget _toolPanel(BuildContext context, List<_Tool> tools, {void Function(_Tool tool)? onTap}) => Container(padding: const EdgeInsets.fromLTRB(7, 10, 7, 10), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(25), boxShadow: const [BoxShadow(color: Color(0x0C000000), blurRadius: 18, offset: Offset(0, 6))]), child: GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: tools.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, mainAxisExtent: 106, crossAxisSpacing: 2, mainAxisSpacing: 2), itemBuilder: (context, index) { final tool = tools[index]; return InkWell(onTap: () => onTap?.call(tool), borderRadius: BorderRadius.circular(18), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 3, vertical: 7), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 54, height: 54, decoration: BoxDecoration(gradient: LinearGradient(colors: [tool.color.withValues(alpha: 0.14), tool.color.withValues(alpha: 0.06)]), shape: BoxShape.circle), child: Icon(tool.icon, color: tool.color, size: 29)), const SizedBox(height: 8), Text(tool.label, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11.5, height: 1.12, fontWeight: FontWeight.w800, color: Color(0xFF2A2730)))]))); });
  }
}
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  @override
  void dispose() { nameController.dispose(); bioController.dispose(); countryController.dispose(); super.dispose(); }

  Future<void> editProfile(Map<String, dynamic> data) async {
    nameController.text = (data['name'] ?? '').toString();
    bioController.text = (data['bio'] ?? '').toString();
    countryController.text = (data['country'] ?? 'India').toString();
    await showDialog<void>(context: context, builder: (dialogContext) => AlertDialog(title: const Text('Edit Profile'), content: SingleChildScrollView(child: Column(children: [TextField(controller: nameController, decoration: const InputDecoration(labelText: 'Name')), TextField(controller: bioController, maxLines: 2, decoration: const InputDecoration(labelText: 'Bio')), TextField(controller: countryController, decoration: const InputDecoration(labelText: 'Country'))])), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Cancel')), FilledButton(onPressed: busy ? null : () async { final name = nameController.text.trim(); if (name.isEmpty) { msg(context, 'Name is required'); return; } setState(() => busy = true); try { await FirebaseFirestore.instance.collection('users').doc(widget.userId).set({'name': name, 'bio': bioController.text.trim(), 'country': countryController.text.trim().isEmpty ? 'India' : countryController.text.trim(), 'updatedAt': FieldValue.serverTimestamp()}, SetOptions(merge: true)); if (FirebaseAuth.instance.currentUser?.uid == widget.userId) await FirebaseAuth.instance.currentUser?.updateDisplayName(name); if (mounted) { Navigator.pop(dialogContext); msg(context, 'Profile updated'); } } catch (e) { if (mounted) msg(context, 'Unable to update profile: $e'); } finally { if (mounted) setState(() => busy = false); } }, child: const Text('Save'))]));
  }
  ...
  Future<void> toggleFollow() async {
    ...
    setState(() => busy = true);
    try {
      final meRef = FirebaseFirestore.instance.collection('users').doc(me);
      final targetRef = FirebaseFirestore.instance.collection('users').doc(widget.userId);
      final followingRef = meRef.collection('following').doc(widget.userId);
      final followerRef = targetRef.collection('followers').doc(me);
      final existing = await followingRef.get();
      final batch = FirebaseFirestore.instance.batch();
      if (existing.exists) { batch.delete(followingRef); batch.delete(followerRef); } else { final payload = {'uid': me, 'createdAt': FieldValue.serverTimestamp()}; batch.set(followingRef, {'uid': widget.userId, 'createdAt': FieldValue.serverTimestamp()}); batch.set(followerRef, payload); }
      await batch.commit();
    } catch (e) { if (mounted) msg(context, 'Follow action failed: $e'); } finally { if (mounted) setState(() => busy = false); }
  }

  @override
  Widget build(BuildContext context) {
    final me = FirebaseAuth.instance.currentUser?.uid;
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection('users').doc(widget.userId).snapshots(), builder: (context, snap) {
      final data = snap.data?.data() ?? {};
      final name = (data['name'] ?? 'WikaLive User').toString();
      final bio = (data['bio'] ?? 'Welcome to WikaLive').toString();
      final country = (data['country'] ?? 'India').toString();
      final followingQuery = FirebaseFirestore.instance.collection('users').doc(widget.userId).collection('following').snapshots();
      final followersQuery = FirebaseFirestore.instance.collection('users').doc(widget.userId).collection('followers').snapshots();
      return Scaffold(appBar: AppBar(title: Text(widget.isOwnProfile ? 'My Profile' : 'Profile'), actions: [if (widget.isOwnProfile) IconButton(onPressed: () => editProfile(data), icon: const Icon(Icons.edit_rounded))]), body: ListView(padding: const EdgeInsets.all(18), children: [Container(padding: const EdgeInsets.all(22), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF2A1C43), Color(0xFF9B55E8)]), borderRadius: BorderRadius.circular(26)), child: Column(children: [const CircleAvatar(radius: 48, child: Icon(Icons.person_rounded, size: 54)), const SizedBox(height: 12), Text(name, style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)), const SizedBox(height: 5), Text('ID: ${widget.userId.length > 8 ? widget.userId.substring(0, 8).toUpperCase() : widget.userId.toUpperCase()}', style: const TextStyle(color: Colors.white70)), const SizedBox(height: 8), Text(bio, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white)), const SizedBox(height: 8), Text('🌍 $country', style: const TextStyle(color: Colors.white70)), if (!widget.isOwnProfile && me != null && me != widget.userId) StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection('users').doc(me).collection('following').doc(widget.userId).snapshots(), builder: (context, followSnap) { final isFollowing = followSnap.data?.exists ?? false; return Padding(padding: const EdgeInsets.only(top: 14), child: FilledButton(onPressed: busy ? null : toggleFollow, child: Text(isFollowing ? 'Following' : 'Follow'))); })])), const SizedBox(height: 16), StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: followingQuery, builder: (context, f1) { return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: followersQuery, builder: (context, f2) { final following = f1.data?.docs.length ?? 0; final followers = f2.data?.docs.length ?? 0; return Row(children: [Expanded(child: _profileStat(context, '$following', 'Following', FollowingPage(userId: widget.userId))), Expanded(child: _profileStat(context, '$followers', 'Followers', FollowersPage(userId: widget.userId)))]); }); }), const SizedBox(height: 14), if (widget.isOwnProfile) FilledButton.icon(onPressed: () => editProfile(data), icon: const Icon(Icons.edit_rounded), label: const Text('Edit Profile'))]);
    });
  }

  Widget _profileStat(BuildContext context, String value, String label, Widget page) => InkWell(onTap: () => openPage(context, page), child: Container(margin: const EdgeInsets.all(5), padding: const EdgeInsets.symmetric(vertical: 18), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18)), child: Column(children: [Text(value, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(label, style: const TextStyle(color: Colors.black54, fontWeight: FontWeight.w700))])));
}

class FollowingPage extends StatelessWidget {
  final String userId;
  const FollowingPage({super.key, required this.userId});
  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Following')), body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection('users').doc(userId).collection('following').snapshots(), builder: (context, snap) => _UserRelationList(snapshot: snap, empty: 'Not following anyone yet.')));
}

class FollowersPage extends StatelessWidget {
  final String userId;
  const FollowersPage({super.key, required this.userId});
  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Followers')), body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection('users').doc(userId).collection('followers').snapshots(), builder: (context, snap) => _UserRelationList(snapshot: snap, empty: 'No followers yet.', canRemove: userId == FirebaseAuth.instance.currentUser?.uid, ownerId: userId)));
}

class _UserRelationList extends StatelessWidget {
  final AsyncSnapshot<QuerySnapshot<Map<String, dynamic>>> snapshot;
  final String empty;
  final bool canRemove;
  final String? ownerId;
  const _UserRelationList({required this.snapshot, required this.empty, this.canRemove = false, this.ownerId});
  @override Widget build(BuildContext context) {
    if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
    final docs = snapshot.data?.docs ?? [];
    if (docs.isEmpty) return Center(child: Text(empty));
    return ListView.separated(padding: const EdgeInsets.all(16), itemCount: docs.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (context, index) { final uid = docs[index].id; return FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(future: FirebaseFirestore.instance.collection('users').doc(uid).get(), builder: (context, userSnap) { final data = userSnap.data?.data() ?? {}; final name = (data['name'] ?? 'WikaLive User').toString(); return Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.person_rounded)), title: Text(name, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('ID: ${uid.length > 8 ? uid.substring(0, 8).toUpperCase() : uid.toUpperCase()}'), onTap: () => openPage(context, ProfilePage(userId: uid)), trailing: canRemove ? IconButton(icon: const Icon(Icons.person_remove_outlined), onPressed: () async { final me = FirebaseAuth.instance.currentUser?.uid; if (me == null || ownerId == null) return; final batch = FirebaseFirestore.instance.batch(); batch.delete(FirebaseFirestore.instance.collection('users').doc(me).collection('followers').doc(uid)); batch.delete(FirebaseFirestore.instance.collection('users').doc(uid).collection('following').doc(me)); await batch.commit(); }) : null)); }); });
  }
}
...
class DiamondWithdrawalPage extends StatefulWidget {
  const DiamondWithdrawalPage({super.key});

  @override
  State<DiamondWithdrawalPage> createState() => _DiamondWithdrawalPageState();
}

class _DiamondWithdrawalPageState extends State<DiamondWithdrawalPage> {
  final controller = TextEditingController();
  bool loading = false;
  ...
  Future<void> submitWithdrawal(int amount) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) {
      msg(context, 'Please login again');
      return;
    ...
    if (amount <= 0) {
      msg(context, 'Enter a valid diamond amount');
      return;
    }
    ...
    try {
      ...
      if (amount > diamonds) {
        if (mounted) {
          setState(() => loading = false);
          msg(context, 'Not enough diamonds');
        }
        return;
      }

      final requestRef = FirebaseFirestore.instance.collection('diamond_withdrawals').doc();
      final batch = FirebaseFirestore.instance.batch();
      ...
      batch.update(userRef, {'diamonds': FieldValue.increment(-amount)});
      await batch.commit();
      ...
      await showDialog<void>(
        context: context,
        builder: (dialogContext) => AlertDialog(
          title: const Text('Diamond withdrawal submitted'),
          content: Text('$amount diamonds have been reserved for your withdrawal request.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(dialogContext),
              child: const Text('OK'),
      ...
      if (mounted) {
        setState(() => loading = false);
        msg(context, 'Unable to submit withdrawal: $e');
      }
    }
  }
          ...
          return ListView(
            ...
            children: [
              Container(
                padding: const EdgeInsets.all(22),
                decoration: BoxDecoration(
                  gradient: const LinearGradient(colors: [Color(0xFFE5D1FF), Color(0xFFF5EFFF)]),
                  borderRadius: BorderRadius.circular(22),
                ),
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 8),
                    const Text('Available Diamonds', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
                    const SizedBox(height: 4),
                    Text('$diamonds', style: const TextStyle(fontSize: 34, fontWeight: FontWeight.w900)),
                  ],
                ),
              ),
              const SizedBox(height: 18),
              ...
              SizedBox(
                ...
                child: FilledButton.icon(
                  onPressed: loading ? null : () => submitWithdrawal(int.tryParse(controller.text.trim()) ?? 0),
                  icon: const Icon(Icons.account_balance_outlined),
                  label: const Text('Request Diamond Withdrawal'),
                ),
              ),
              const SizedBox(height: 14),
              const Text(
                'Your request is saved in Firestore with pending status. Actual bank/UPI payout must be completed by the app operator/admin payout system.',
                style: TextStyle(color: Colors.black54),
              ),
            ],
          );
  ...
  Future<void> verifyCode() async {
    ...
    try {
      ...
      if (user != null) {
        final ref = FirebaseFirestore.instance.collection('users').doc(user.uid);
        if (!(await ref.get()).exists) {
          await ref.set({'uid': user.uid, 'name': 'WikaLive User', 'phone': user.phoneNumber ?? phone.text.trim(), 'coins': 0, 'diamonds': 0, 'createdAt': FieldValue.serverTimestamp(), 'updatedAt': FieldValue.serverTimestamp()});
        }
      }
