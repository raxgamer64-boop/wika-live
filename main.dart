...
class _WikaLiveChatComposer extends StatefulWidget {
  final TextEditingController controller;
  final Future<void> Function() onSend;
  const _WikaLiveChatComposer({
    required this.controller,
    required this.onSend,
  });

  @override
  State<_WikaLiveChatComposer> createState() => _WikaLiveChatComposerState();
}

class _WikaLiveChatComposerState extends State<_WikaLiveChatComposer> {
  ...
  Widget build(BuildContext context) {
    ...
    final emojis = _categories[_category];
    return AnimatedPadding(
      ...
      child: Container(
        ...
        decoration: const BoxDecoration(
          ...
          border: Border(top: BorderSide(color: Color(0x403F3B4A))),
        ),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
                ...
                children: [
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: TextField(
                              ...
                              decoration: const InputDecoration(
                                ...
                                hintStyle: TextStyle(color: Color(0xFF9A98A4)),
                          ...
                          IconButton(
                            ...
                            icon: const Icon(Icons.emoji_emotions_rounded, color: Colors.white, size: 26),
                          ),
                        ],
                      ),
                    ),
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: const BoxDecoration(
                        shape: BoxShape.circle,
                        gradient: LinearGradient(colors: [Color(0xFF8B31FF), Color(0xFFFF31B6)]),
                        boxShadow: [BoxShadow(color: Color(0x889B2EFF), blurRadius: 14, spreadRadius: 1)],
                      ),
                      child: const Icon(Icons.arrow_upward_rounded, color: Colors.white, size: 29),
                    ),
                  ),
                ],
              ),
            ...
            if (_showPicker) Row(
              children: [
                _composerTab('Emoji', Icons.emoji_emotions_outlined),
                _composerTab('Stickers', Icons.sticky_note_2_outlined),
                _composerTab('GIF', Icons.gif_rounded),
                _composerTab('Effects', Icons.auto_awesome_outlined),
              ],
            ),
            if (_showPicker && _tab == 'Emoji') ...[
              SizedBox(
                height: 48,
                child: ListView.separated(
                  ...
                  itemBuilder: (_, index) {
                    final icons = [Icons.emoji_emotions_rounded, Icons.favorite_rounded, Icons.thumb_up_alt_rounded, Icons.celebration_rounded];
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        height: 34,
                        decoration: BoxDecoration(
                          color: _category == index ? const Color(0xFF7E2DCC) : const Color(0xFF1B1C22),
                          borderRadius: BorderRadius.circular(17),
                        ),
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Icon(
                      _tab == 'Stickers' ? Icons.sticky_note_2_rounded : _tab == 'GIF' ? Icons.gif_rounded : Icons.auto_awesome_rounded,
                      color: const Color(0xFFD34CFF),
                      size: 30,
                    ),
                    const SizedBox(width: 10),
                    Expanded(
                      child: Text(
                        _tab == 'Stickers' ? 'Stickers will appear here.' : _tab == 'GIF' ? 'GIF picker will appear here.' : 'Effects will appear here.',
  ...
  Widget _composerTab(String label, IconData icon) {
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(icon, color: active ? const Color(0xFFD44CFF) : Colors.white70, size: 19),
              const SizedBox(height: 3),
              Text(
                label,
                style: TextStyle(
...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
  ...
  Future<void> _toggleHostFollow() async {
    final me = FirebaseAuth.instance.currentUser;
    final hostUid = _resolvedHostUid;
    if (me == null || hostUid == null || hostUid.isEmpty || me.uid == hostUid) return;

    final myRef = FirebaseFirestore.instance
        .collection('users').doc(me.uid).collection('following').doc(hostUid);
    final hostFollowerRef = FirebaseFirestore.instance
        .collection('users').doc(hostUid).collection('followers').doc(me.uid);

    try {
      final existing = await myRef.get();
      if (existing.exists) {
        await myRef.delete();
        await hostFollowerRef.delete();
      } else {
        final hostSnap = await FirebaseFirestore.instance.collection('users').doc(hostUid).get();
        final hostData = hostSnap.data() ?? <String, dynamic>{};
        await myRef.set({
          'uid': hostUid,
          'name': (hostData['name'] ?? widget.hostName).toString(),
          'photoUrl': (hostData['photoUrl'] ?? widget.hostPhotoUrl ?? '').toString(),
          'createdAt': FieldValue.serverTimestamp(),
        });
        await hostFollowerRef.set({
          'uid': me.uid,
          'name': (me.displayName ?? 'WikaLive User').toString(),
          'photoUrl': (me.photoURL ?? '').toString(),
          'createdAt': FieldValue.serverTimestamp(),
        });
      }
    } catch (e) {
      debugPrint('WikaLive follow toggle failed: $e');
    }
  }
      ...
      builder: (context, roomSnapshot) {
        ...
        return DefaultTextStyle.merge(
          ...
          child: Stack(
            fit: StackFit.expand,
            children: [
            // COMPACT SINGLE-ROW HEADER: host, guardian, ranking, viewers and close.
            // The widths are responsive so the host name never competes with the
            // guardian badge or the right-side controls.
            Positioned(
              top: top + 18,
              left: 10,
              right: 10,
            ...
            // FUNCTIONAL ZEGO GUEST/CO-HOST CONTROL. Keep it just below the
            // top header/diamond area instead of floating in the middle.
            Positioned(
              right: 12,
              top: top + 174,
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                      colors: [Color(0xFF6B24D8), Color(0xFFA83BFF)],
                    ),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Row(
        ...
        return SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final amount = d['diamondsSent'] is num ? (d['diamondsSent'] as num).toInt() : 0;
              return _rankDataAvatar(index + 1, name, photo, amount);
            },
          ),
        );
      },
  ...
  Widget _rankDataAvatar(int rank, String name, String photo, int amount) {
    final ring = rank == 1
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: ring.withOpacity(.35), blurRadius: 8)],
                  ),
          ...
          Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Icon(Icons.diamond_rounded, color: Color(0xFFD34CFF), size: 9),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: const LinearGradient(
                      ...
                      colors: [Color(0xFFFFD36A), Color(0xFFB43DFF), Color(0xFF5B17A8)],
                    ),
                    border: Border.all(color: const Color(0xFFEAC5FF), width: 1.5),
                    boxShadow: const [
                      BoxShadow(color: Color(0xCC9B36FF), blurRadius: 18, spreadRadius: 2),
                      BoxShadow(color: Color(0x88FFB84D), blurRadius: 9),
                    ],
                  ),
                ),
                Container(
                  ...
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    color: const Color(0xFF32105F),
                    border: Border.all(color: const Color(0xFFFFD86E), width: 1.2),
                  ),
                  child: Icon(Icons.shield_rounded, color: Colors.white, size: d * .43),
                ),
                Positioned(
                  top: -4,
                  child: Icon(
                    ...
                    size: d * .42,
                    shadows: const [Shadow(color: Color(0xFFFF8A00), blurRadius: 8)],
                  ),
                ),
              ],
            ),
  ...
  Future<void> _openChatComposer() async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) {
        return _WikaLiveChatComposer(
          controller: _chatController,
          onSend: () async {
            await _sendMessage();
            if (sheetContext.mounted && _chatController.text.trim().isEmpty) {
              // Keep the composer open after sending so users can continue chatting.
            }
          },
        );
      },
              ...
              const Expanded(
                child: Padding(
                  ...
                  child: Text(
                    ...
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(color: Color(0xFFA9A6B0), fontSize: 14),
                  ),
                ),
              ),
              Container(
