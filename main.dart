...
class _StartLivePageState extends State<StartLivePage> {
  ...
  Future<void> _start() async {
    ...
    try {
      // Firestore profile data is helpful but must not prevent the camera room
      // from opening if an optional profile field is unavailable.
      try {
        final snap = await FirebaseFirestore.instance.collection('users').doc(user.uid).get().timeout(const Duration(seconds: 6));
      ...
      // IMPORTANT: do not mark the user live before entering ZEGO.
      // The host must reach the real ZEGO room first; RealHostLivePage then
      // publishes the live status while the host room is active.
      ...
      // Clean up the user's live status after leaving the ZEGO room.
      try {
        ...
        child: Column(
          ...
          children: [
            ...
            FilledButton.icon(
              ...
              label: Padding(
                padding: const EdgeInsets.symmetric(vertical: 14),
                child: Text(starting ? 'STARTING LIVE…' : 'START LIVE NOW', style: const TextStyle(fontWeight: FontWeight.w900)),
              ),
            ),
            const SizedBox(height: 20),
          ],
        ...
        onLeaveConfirmation: (
          ...
          final close = await showDialog<bool>(
            ...
            builder: (dialogContext) => AlertDialog(
              ...
              content: const Text(
                'Do you want to close this live?\n\nCancel = continue live\nClose Live = end the live now',
  ...
  Widget _guestSlot(BuildContext context, int index, {bool plus = false}) {
    return StreamBuilder<List<ZegoUIKitUser>>(
      ...
      builder: (context, snapshot) {
        ...
        final guests = users.where((u) => u.isCameraOn || u.isMicrophoneOn).toList();
