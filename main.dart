...
class _EditProfilePageState extends State<EditProfilePage>{final picker=ImagePicker();final name=TextEditingController();final bio=TextEditingController();final birthday=TextEditingController();final tag=TextEditingController();final language=TextEditingController();final country=TextEditingController();final photo=TextEditingController();List<String> photos=[];bool saving=false;@override void initState(){super.initState();load();}Future<void> load()async{final u=FirebaseAuth.instance.currentUser;if(u==null)return;name.text=u.displayName??'';final d=(await FirebaseFirestore.instance.collection('users').doc(u.uid).get()).data()??{};bio.text=(d['bio']??'').toString();birthday.text=(d['birthday']??'').toString();tag.text=(d['tag']??'').toString();language.text=(d['language']??'').toString();country.text=(d['country']??'India').toString();photos=d['photos'] is List?List<String>.from((d['photos'] as List).map((e)=>e.toString())):[];if(mounted)setState((){});}Future<void> addPhoto() async {
  final u=FirebaseAuth.instance.currentUser;
  ...
  }catch(e){if(mounted)msg(context,'Photo upload failed: $e');}finally{if(mounted)setState(()=>saving=false);}
}
Future<void>save()async{final u=FirebaseAuth.instance.currentUser;if(u==null)return;setState(()=>saving=true);try{await u.updateDisplayName(name.text.trim());await FirebaseFirestore.instance.collection('users').doc(u.uid).set({'name':name.text.trim(),'bio':bio.text.trim(),'birthday':birthday.text.trim(),'tag':tag.text.trim(),'language':language.text.trim(),'country':country.text.trim(),'photos':photos,'photoUrl':photos.isEmpty?'':photos.first,'updatedAt':FieldValue.serverTimestamp()},SetOptions(merge:true));if(mounted){msg(context,'Profile updated');Navigator.pop(context);}}catch(e){if(mounted)msg(context,'Update failed: $e');}finally{if(mounted)setState(()=>saving=false);}}@override Widget build(BuildContext c)=>Scaffold(appBar:AppBar(title:const Text('Edit Profile')),body:ListView(padding:const EdgeInsets.all(18),children:[TextField(controller:name,decoration:const InputDecoration(labelText:'Name',prefixIcon:Icon(Icons.person),filled:true)),const SizedBox(height:10),TextField(controller:bio,maxLines:3,decoration:const InputDecoration(labelText:'Bio',prefixIcon:Icon(Icons.info),filled:true)),const SizedBox(height:10),TextField(controller:birthday,decoration:const InputDecoration(labelText:'Birthday',hintText:'DD/MM/YYYY',prefixIcon:Icon(Icons.cake),filled:true)),const SizedBox(height:10),TextField(controller:tag,decoration:const InputDecoration(labelText:'Tag',prefixIcon:Icon(Icons.tag),filled:true)),const SizedBox(height:10),TextField(controller:language,decoration:const InputDecoration(labelText:'Language',prefixIcon:Icon(Icons.language),filled:true)),const SizedBox(height:10),TextField(controller:country,decoration:const InputDecoration(labelText:'Country',prefixIcon:Icon(Icons.flag),filled:true)),const SizedBox(height:16),const Text('Profile photos (up to 5)',style:TextStyle(fontWeight:FontWeight.w900)),const SizedBox(height:8),...photos.asMap().entries.map((e)=>ListTile(leading:CircleAvatar(backgroundImage:NetworkImage(e.value)),title:Text(e.value,maxLines:1,overflow:TextOverflow.ellipsis),trailing:IconButton(onPressed:()=>setState(()=>photos.removeAt(e.key)),icon:const Icon(Icons.delete_outline)))),SizedBox(height:50,child:OutlinedButton.icon(onPressed:saving||photos.length>=5?null:addPhoto,icon:const Icon(Icons.photo_library_outlined),label:Text(photos.length>=5?'5 photos added':'Add photo from gallery'))),
const SizedBox(height:8),
TextField(controller:photo,decoration:InputDecoration(labelText:'Or add photo URL',suffixIcon:IconButton(onPressed:(){if(photo.text.trim().isNotEmpty&&photos.length<5)setState((){photos.add(photo.text.trim());photo.clear();});},icon:const Icon(Icons.add_photo_alternate)))),const SizedBox(height:20),SizedBox(height:52,child:FilledButton(onPressed:saving?null:save,child:Text(saving?'Saving...':'Save Profile')))]));}

class RankingPage extends StatefulWidget { const RankingPage({super.key}); @override State<RankingPage> createState()=>_RankingPageState(); }
class _RankingPageState extends State<RankingPage>{String type='Live';String period='Day';final types=['Live','Sender','Party','Game'];final periods=['Hour','Day','Week'];@override Widget build(BuildContext c)=>Scaffold(appBar:AppBar(title:const Text('Rankings')),body:Column(children:[Padding(padding:const EdgeInsets.all(12),child:Row(children:[for(final x in types)Expanded(child:Padding(padding:const EdgeInsets.symmetric(horizontal:2),child:ChoiceChip(label:Text(x),selected:type==x,onSelected:(_){setState(()=>type=x);}))),])),SingleChildScrollView(scrollDirection:Axis.horizontal,padding:const EdgeInsets.symmetric(horizontal:12),child:Row(children:[for(final x in periods)Padding(padding:const EdgeInsets.only(right:8),child:ChoiceChip(label:Text(x),selected:period==x,onSelected:(_){setState(()=>period=x);})),])),Expanded(child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('users').snapshots(),builder:(c,s){final docs=[...(s.data?.docs??[])];String field=type=='Live'?'giftReceived':type=='Sender'?'giftSent':type=='Party'?'partyGiftReceived':'gameSpent';docs.sort((a,b)=>(b.data()[field] is num?b.data()[field] as num:0).compareTo(a.data()[field] is num?a.data()[field] as num:0));final top=docs.take(50).toList();return ListView.builder(itemCount:top.length,itemBuilder:(c,i){final d=top[i].data();return ListTile(leading:CircleAvatar(child:Text('${i+1}')),title:Text((d['name']??'User').toString()),trailing:Text('${d[field]??0}'));});}))]));}
