...
class KingOfKingsPage extends StatelessWidget {
  const KingOfKingsPage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('King of Kings')),
      body: ListView(
        padding: const EdgeInsets.fromLTRB(16, 12, 16, 28),
        children: [
          _hubHero('💎', 'King of Kings', 'Royal status, ranking and exclusive privileges'),
          const SizedBox(height: 16),
          _hubSection('MY ROYAL STATUS', [
            _hubItem(context, Icons.workspace_premium_rounded, 'My Level & Status', 'View level, progress and current title', const SimplePage(title: 'My Level & Status', items: ['Current level', 'Level progress', 'Level benefits', 'Level history'])),
            _hubItem(context, Icons.diamond_rounded, 'Diamond Benefits', 'See diamond-based benefits and rewards', const SimplePage(title: 'Diamond Benefits', items: ['Diamond balance', 'Benefits', 'Reward rules', 'Diamond history'])),
          ]),
          const SizedBox(height: 12),
          _hubSection('PRIVILEGES', [
            _hubItem(context, Icons.card_giftcard_rounded, 'Exclusive Gifts', 'Special gifts and gift privileges', const SimplePage(title: 'Exclusive Gifts', items: ['Special gifts', 'Gift collection', 'Gift history'])),
            _hubItem(context, Icons.auto_awesome_rounded, 'VIP Privileges', 'Badges, frames and special access', const SimplePage(title: 'VIP Privileges', items: ['VIP badge', 'Profile frame', 'Room privileges', 'Special access'])),
            _hubItem(context, Icons.shield_rounded, 'Guardian Privileges', 'Guardian status and benefits', const SimplePage(title: 'Guardian Privileges', items: ['Guardian status', 'Guard a host', 'Guardian benefits', 'Guardian history'])),
          ]),
          const SizedBox(height: 12),
          _hubSection('RANKING & REWARDS', [
            _hubItem(context, Icons.emoji_events_rounded, 'King Ranking', 'Global and country rankings', const RankingPage()),
            _hubItem(context, Icons.calendar_month_rounded, 'Daily / Weekly Rewards', 'Check available royal rewards', const SimplePage(title: 'Royal Rewards', items: ['Daily rewards', 'Weekly rewards', 'Claimed rewards', 'Reward history'])),
          ]),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        padding: const EdgeInsets.fromLTRB(16, 12, 16, 28),
        children: [
          _hubHero('🎁', 'Invite & Earn', 'Invite friends and track your referral rewards'),
          const SizedBox(height: 16),
          _hubSection('INVITE', [
            _hubItem(context, Icons.link_rounded, 'My Invite Code', 'Copy and share your personal code', const SimplePage(title: 'My Invite Code', items: ['Your invite code', 'Copy code', 'Share invite link'])),
            _hubItem(context, Icons.person_add_alt_1_rounded, 'Invite Friends', 'Share your invite link with friends', const SimplePage(title: 'Invite Friends', items: ['Share link', 'Share code', 'Invite contacts'])),
          ]),
          const SizedBox(height: 12),
          _hubSection('EARNINGS', [
            _hubItem(context, Icons.account_balance_wallet_rounded, 'Referral Earnings', 'See your earned referral rewards', const SimplePage(title: 'Referral Earnings', items: ['Total earnings', 'Pending rewards', 'Available rewards', 'Earnings history'])),
            _hubItem(context, Icons.card_giftcard_rounded, 'Reward Rules', 'Understand how invitation rewards work', const SimplePage(title: 'Reward Rules', items: ['Invite requirements', 'Reward levels', 'Claim rules', 'Terms'])),
          ]),
          const SizedBox(height: 12),
          _hubSection('HISTORY', [
            _hubItem(context, Icons.people_alt_rounded, 'Invited Users', 'Users invited through your link', const SimplePage(title: 'Invited Users', items: ['All invited users', 'Joined users', 'Pending users'])),
            _hubItem(context, Icons.history_rounded, 'Invitation History', 'Track every invitation and reward', const SimplePage(title: 'Invitation History', items: ['Invitation history', 'Reward history', 'Status'])),
          ]),
        ],
      ),
    );
...
Widget _hubHero(String icon, String title, String subtitle) {
  return Container(
    padding: const EdgeInsets.fromLTRB(20, 24, 20, 22),
    decoration: BoxDecoration(
      gradient: const LinearGradient(colors: [Color(0xFF241332), Color(0xFF6B2F73), Color(0xFFB34D9B)]),
      borderRadius: BorderRadius.circular(26),
      boxShadow: const [BoxShadow(color: Color(0x221B1025), blurRadius: 18, offset: Offset(0, 8))],
    ),
    child: Column(children: [
      ...
      const SizedBox(height: 6),
      Text(subtitle, textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFFE9DDED), fontSize: 12, fontWeight: FontWeight.w600)),
    ]),
  );
}

Widget _hubSection(String title, List<Widget> children) {
  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Padding(padding: const EdgeInsets.only(left: 4, bottom: 8), child: Text(title, style: const TextStyle(fontSize: 12, letterSpacing: .8, fontWeight: FontWeight.w900, color: Color(0xFF756A80)))),
    Container(
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), boxShadow: const [BoxShadow(color: Color(0x0C000000), blurRadius: 16, offset: Offset(0, 5))]),
      child: Column(children: List.generate(children.length, (i) => Column(children: [children[i], if (i < children.length - 1) const Divider(height: 1, indent: 70, endIndent: 14)]))),
    ),
  ]);
}

Widget _hubItem(BuildContext context, IconData icon, String title, String subtitle, Widget page) {
  return ListTile(
    contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 5),
    leading: Container(width: 46, height: 46, decoration: BoxDecoration(color: const Color(0xFFF1E7FA), borderRadius: BorderRadius.circular(15)), child: Icon(icon, color: const Color(0xFF9654D5))),
    title: Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w900)),
    subtitle: Padding(padding: const EdgeInsets.only(top: 3), child: Text(subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, color: Color(0xFF8B8593), fontWeight: FontWeight.w600))),
    trailing: const Icon(Icons.chevron_right_rounded, color: Color(0xFF55505A)),
