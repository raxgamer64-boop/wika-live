...
class _UserSearchPageState extends State<UserSearchPage> {
  ...
  Future<void> _search() async {
    ...
    try {
      // User ID is derived from the Firebase UID, so it also works for
      // older accounts whose user document was created before the numeric
      // userId field was added.
      final snap = await FirebaseFirestore.instance.collection('users').get();
      final matches = snap.docs.where((doc) {
        final data = doc.data();
        final stored = (data['userId'] ?? '').toString().trim();
        final numericId = wikaNumericUserId(doc.id);
        return stored == query || numericId == query;
      }).toList();
        ...
        if (searching)
          ...
          Column(
            children: results.map((d) {
              final data = d.data();
              // Always display the immutable numeric ID derived from the
              // Firebase UID. This keeps Search and My Space identical.
              final id = wikaNumericUserId(d.id);
              return ListTile(
                leading: CircleAvatar(
