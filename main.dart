...
class MeScreen extends StatelessWidget {
  ...
  Widget _statsCard(BuildContext context, String uid) {
    return Container(
      ...
      child: Row(children: [
        _statTap(context, '0', 'Friends', const FeaturePage(title: 'Friends')),
        _divider(),
        ...
        _divider(),
        _statTap(context, '0', 'Visitors', const FeaturePage(title: 'Visitors')),
      ]),
    );
  }
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          return ListView(
            padding: const EdgeInsets.all(20),
            children: [
              const CircleAvatar(radius: 55, child: Icon(Icons.person_rounded, size: 60)),
              const SizedBox(height: 14),
              Text(name, textAlign: TextAlign.center, style: const TextStyle(fontSize: 25, fontWeight: FontWeight.w900)),
              const SizedBox(height: 5),
              Text('ID: ' + user.uid, textAlign: TextAlign.center, style: const TextStyle(color: Colors.black45)),
              if (bio.isNotEmpty) ...[
                const SizedBox(height: 12),
                Text(bio, textAlign: TextAlign.center),
              ],
              const SizedBox(height: 22),
              menu(context, 'Edit Profile', Icons.edit_outlined, const EditProfilePage()),
              menu(context, 'Following', Icons.person_add_alt_1_rounded, const FollowingPage()),
              menu(context, 'Followers', Icons.people_alt_outlined, const FollowersPage()),
            ],
          );
        },
      ),
...
class _EditProfilePageState extends State<EditProfilePage> { final name=TextEditingController(); final bio=TextEditingController(); bool loading=false; @override void initState(){super.initState();name.text=FirebaseAuth.instance.currentUser?.displayName??'';} @override void dispose(){name.dispose();bio.dispose();super.dispose();} Future<void> save() async { final u=FirebaseAuth.instance.currentUser; if(u==null)return; if(name.text.trim().isEmpty){msg(context,'Enter your name');return;} setState(()=>loading=true); try { await u.updateDisplayName(name.text.trim()); await FirebaseFirestore.instance.collection('users').doc(u.uid).set({'name':name.text.trim(),'bio':bio.text.trim(),'updatedAt':FieldValue.serverTimestamp()},SetOptions(merge:true)); if(mounted){msg(context,'Profile updated');Navigator.pop(context);}} catch(e){if(mounted)msg(context,'Could not update profile: $e');} finally {if(mounted)setState(()=>loading=false);} } @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('Edit Profile')),body:ListView(padding:const EdgeInsets.all(20),children:[TextField(controller:name,decoration:const InputDecoration(labelText:'Name',prefixIcon:Icon(Icons.person_outline),filled:true)),const SizedBox(height:14),TextField(controller:bio,maxLines:3,decoration:const InputDecoration(labelText:'Bio',prefixIcon:Icon(Icons.info_outline),filled:true)),const SizedBox(height:20),SizedBox(height:52,child:FilledButton(onPressed:loading?null:save,child:Text(loading?'Saving...':'Save Profile')))])); }
class FollowingPage extends StatelessWidget { const FollowingPage({super.key}); @override Widget build(BuildContext context){final u=FirebaseAuth.instance.currentUser;if(u==null)return const Scaffold(body:Center(child:Text('Please login again')));return Scaffold(appBar:AppBar(title:const Text('Following')),body:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('users').doc(u.uid).collection('following').orderBy('createdAt',descending:true).snapshots(),builder:(context,s){if(s.connectionState==ConnectionState.waiting)return const Center(child:CircularProgressIndicator());final docs=s.data?.docs??[];if(docs.isEmpty)return const Center(child:Text('You are not following anyone yet'));return ListView.builder(itemCount:docs.length,itemBuilder:(context,i){final d=docs[i].data();final id=(d['uid']??docs[i].id).toString();return ListTile(leading:const CircleAvatar(child:Icon(Icons.person)),title:Text((d['name']??'User').toString()),subtitle:Text(id),onTap:()=>openPage(context,UserProfilePage(uid:id)));});}));}}
class FollowersPage extends StatelessWidget { const FollowersPage({super.key}); @override Widget build(BuildContext context){final u=FirebaseAuth.instance.currentUser;if(u==null)return const Scaffold(body:Center(child:Text('Please login again')));return Scaffold(appBar:AppBar(title:const Text('Followers')),body:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('users').doc(u.uid).collection('followers').orderBy('createdAt',descending:true).snapshots(),builder:(context,s){if(s.connectionState==ConnectionState.waiting)return const Center(child:CircularProgressIndicator());final docs=s.data?.docs??[];if(docs.isEmpty)return const Center(child:Text('No followers yet'));return ListView.builder(itemCount:docs.length,itemBuilder:(context,i){final d=docs[i].data();final id=(d['uid']??docs[i].id).toString();return ListTile(leading:const CircleAvatar(child:Icon(Icons.person)),title:Text((d['name']??'User').toString()),subtitle:Text(id),onTap:()=>openPage(context,UserProfilePage(uid:id)));});}));}}
class UserProfilePage extends StatefulWidget { final String uid; const UserProfilePage({super.key,required this.uid}); @override State<UserProfilePage> createState()=>_UserProfilePageState(); }
class _UserProfilePageState extends State<UserProfilePage> { bool following=false; bool loading=false; @override void initState(){super.initState();load();} Future<void> load()async{final me=FirebaseAuth.instance.currentUser?.uid;if(me==null||me==widget.uid)return;final d=await FirebaseFirestore.instance.collection('users').doc(me).collection('following').doc(widget.uid).get();if(mounted)setState(()=>following=d.exists);} Future<void> toggle()async{final me=FirebaseAuth.instance.currentUser;if(me==null||me.uid==widget.uid)return;setState(()=>loading=true);final db=FirebaseFirestore.instance;final meRef=db.collection('users').doc(me.uid);final targetRef=db.collection('users').doc(widget.uid);final f=meRef.collection('following').doc(widget.uid);final r=targetRef.collection('followers').doc(me.uid);try{if(following){await f.delete();await r.delete();}else{final target=(await targetRef.get()).data()??{};final md=(await meRef.get()).data()??{};final b=db.batch();b.set(f,{'uid':widget.uid,'name':target['name']??'User','createdAt':FieldValue.serverTimestamp()});b.set(r,{'uid':me.uid,'name':md['name']??me.displayName??'User','createdAt':FieldValue.serverTimestamp()});await b.commit();}if(mounted)setState(()=>following=!following);}catch(e){if(mounted)msg(context,'Could not update follow: $e');}finally{if(mounted)setState(()=>loading=false);}} @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('User Profile')),body:StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('users').doc(widget.uid).snapshots(),builder:(context,s){final d=s.data?.data()??{};return ListView(padding:const EdgeInsets.all(20),children:[const CircleAvatar(radius:55,child:Icon(Icons.person,size:60)),const SizedBox(height:14),Text((d['name']??'WikaLive User').toString(),textAlign:TextAlign.center,style:const TextStyle(fontSize:25,fontWeight:FontWeight.w900)),const SizedBox(height:8),Text((d['bio']??'').toString(),textAlign:TextAlign.center),const SizedBox(height:20),FilledButton.icon(onPressed:loading?null:toggle,icon:Icon(following?Icons.check:Icons.person_add_alt_1),label:Text(following?'Following':'Follow'))]);}));}
