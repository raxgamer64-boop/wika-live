...
class _RealHostLivePageState extends State<RealHostLivePage> {
  ...
  Future<void> _markLive() async {
    ...
    try {
      ...
      final photo = (data['photoUrl'] ?? widget.photoUrl ?? '').toString();

      // This document is the realtime public live directory.  It is written
      // only after the ZEGO host page has opened, so a listed room always has
      // a real liveId that an audience member can join.
      await ref.set({
        'uid': user.uid,
        'isLive': true,
