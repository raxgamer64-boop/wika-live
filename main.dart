  ...
  Widget build(BuildContext context) {
    ...
    return ZegoUIKitPrebuiltLiveStreaming(
      ...
      config: ZegoUIKitPrebuiltLiveStreamingConfig.audience(plugins: [ZegoUIKitSignalingPlugin()])
        ...
        ..foreground = _GuestCallOverlay(isHost: false, hostName: host);
...
class _RealHostLivePageState extends State<RealHostLivePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final config = ZegoUIKitPrebuiltLiveStreamingConfig.host(plugins: [ZegoUIKitSignalingPlugin()])
      ...
      ..foreground = _GuestCallOverlay(isHost: true, hostName: widget.name);
...
class _GuestCallOverlay extends StatefulWidget {
  ...
  const _GuestCallOverlay({required this.isHost, required this.hostName});
...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
  ...
  void _toggleCamera() {
    zego.audioVideo.camera.switchState();
  ...
  void _toggleBeauty() {
    ...
    setState(() => beautyOn = next);
  }

  void _sendHeart() {
    setState(() => hearts = (hearts + 1).clamp(0, 5));
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
                ...
                Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text(
                          widget.hostName.isEmpty ? 'jerry ki duniya' : widget.hostName,
                          style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w900),
                        ),
                        const SizedBox(width: 5),
                        const Icon(Icons.verified_rounded, color: Color(0xFFD55CFF), size: 16),
                      ],
                    ),
                    const SizedBox(height: 2),
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: const Color(0xFF8E35C8).withOpacity(.82),
                        borderRadius: BorderRadius.circular(10),
                      ),
                      child: const Text('♛ Lv.32', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w900)),
                    ),
                  ],
                ),
              ],
            ...
            child: Center(
              child: Row(
                ...
                children: [
                  _guardianBadge(),
                  const SizedBox(width: 10),
                  _rankAvatar('1', '58', Icons.person, const Color(0xFFFFD24A)),
                  _rankAvatar('2', '46', Icons.person, const Color(0xFFB7D8FF)),
                  _rankAvatar('3', '39', Icons.person, const Color(0xFFFF9A7A)),
                ],
              ),
            ),
          ),
                ...
                children: [
                  _glass(
                    ...
                    child: const Row(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      SizedBox(width: 6),
                      Text('1.2K', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),
                    ]),
                  ),
                  const SizedBox(width: 7),
                  _roundGlass(Icons.close_rounded, 42, () => _confirmClose(context)),
                ],
              ),
              const SizedBox(height: 9),
              _glass(
                ...
                radius: 22,
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  Text('◆', style: TextStyle(color: Color(0xFFC85CFF), fontSize: 21)),
                  SizedBox(width: 8),
                  Text('5.0K', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 14)),
                  SizedBox(width: 7),
                  Icon(Icons.chevron_right_rounded, color: Colors.white70, size: 19),
                ]),
              ),
            ],
          ),
        ...
        // Top contribution.
        Positioned(
          top: MediaQuery.of(context).padding.top + 91,
          left: 15,
          child: _glass(
            padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 7),
            radius: 18,
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.emoji_events_rounded, color: Color(0xFFFFD75C), size: 19),
              SizedBox(width: 5),
              Text('Top 1', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12)),
              SizedBox(width: 7),
              Text('5.0K', style: TextStyle(color: Color(0xFFFFD75C), fontWeight: FontWeight.w900, fontSize: 12)),
            ]),
          ),
        ),
        ...
        Positioned(
          ...
          child: _guestSlot(context),
        ),

        // Real ZEGO chat messages.
        Positioned(
          left: 14,
          right: 88,
          bottom: 118,
          child: StreamBuilder<List<ZegoInRoomMessage>>(
            ...
            builder: (context, snapshot) {
              final messages = snapshot.data ?? const <ZegoInRoomMessage>[];
              ...
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                mainAxisSize: MainAxisSize.min,
                children: visible.map((m) => _chatBubble(m.user.name, m.message)).toList(),
              );
            },
          ),
        ),
        ...
        Positioned(
          ...
          child: IgnorePointer(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: List.generate(hearts, (i) => const Padding(
                padding: EdgeInsets.only(top: 6),
                child: Icon(Icons.favorite_rounded, color: Color(0xFFFF3CA7), size: 38),
              )),
            ),
          ),
        ),
        ...
        Positioned(
          ...
          child: Row(
            ...
            children: [
              Expanded(
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    IconButton(onPressed: _sendHeart, icon: const Icon(Icons.emoji_emotions_outlined, color: Colors.white, size: 24)),
                  ]),
                ),
              ),
              const SizedBox(width: 8),
              ...
              _bottomButton(Icons.camera_alt_rounded, 'Camera', _toggleCamera, active: cameraOn),
  ...
  Widget _glass({required Widget child, required EdgeInsetsGeometry padding, required double radius}) {
    return Container(
      padding: padding,
      decoration: BoxDecoration(
        color: const Color(0xC9141419),
  ...
  Widget _roundGlass(IconData icon, double size, VoidCallback onTap) {
    return InkWell(
      ...
      child: Container(
        width: size,
        height: size,
        decoration: BoxDecoration(color: const Color(0xC9141419), shape: BoxShape.circle, border: Border.all(color: Colors.white24)),
  ...
  Widget _guardianBadge() {
    return Container(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        gradient: const RadialGradient(colors: [Color(0xFF6E4DFF), Color(0xFF22163B)]),
        border: Border.all(color: const Color(0xFFC86BFF), width: 2),
        boxShadow: const [BoxShadow(color: Color(0x885E39FF), blurRadius: 14)],
      ),
      child: const Icon(Icons.shield_rounded, color: Color(0xFFDDB6FF), size: 36),
    );
  }

  Widget _rankAvatar(String rank, String level, IconData icon, Color ring) {
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Stack(clipBehavior: Clip.none, children: [
          Container(
            width: 43,
            height: 43,
            decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFF282A31), border: Border.all(color: ring, width: 2)),
            child: Icon(icon, color: Colors.white, size: 22),
          ),
          Positioned(left: 13, bottom: -7, child: Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1), decoration: BoxDecoration(color: const Color(0xFF17171C), borderRadius: BorderRadius.circular(8)), child: Text(rank, style: TextStyle(color: ring, fontSize: 9, fontWeight: FontWeight.w900)))),
        ]),
        const SizedBox(height: 7),
        Text('Lv.$level', style: TextStyle(color: ring, fontSize: 9, fontWeight: FontWeight.w900)),
      ]),
    );
  }

  Widget _guestSlot(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        width: 106,
        height: 106,
        decoration: BoxDecoration(color: Colors.black.withOpacity(.20), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white.withOpacity(.65), width: 1.4)),
        child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.add_rounded, color: Colors.white, size: 45),
          SizedBox(height: 2),
          Text('Guest', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w700)),
        ]),
      ),
    );
  }

  Widget _chatBubble(String name, String message) {
    return Container(
      margin: const EdgeInsets.only(bottom: 6),
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
      decoration: BoxDecoration(color: const Color(0xAA15151A), borderRadius: BorderRadius.circular(18)),
      child: RichText(text: TextSpan(children: [
        TextSpan(text: '${name.isEmpty ? 'User' : name}: ', style: const TextStyle(color: Color(0xFFD7B2FF), fontSize: 12, fontWeight: FontWeight.w900)),
        TextSpan(text: message, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
      ])),
    );
  }
  ...
  Widget _bottomButton(IconData icon, String label, VoidCallback onTap, {bool active = true}) {
    return Padding(
      ...
      child: InkWell(
        ...
        borderRadius: BorderRadius.circular(30),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 56, height: 56, decoration: BoxDecoration(color: const Color(0xCC15151A), shape: BoxShape.circle, border: Border.all(color: active ? Colors.white24 : const Color(0xFFFF5C9D), width: 1)), child: Icon(icon, color: active ? Colors.white : const Color(0xFFFF5C9D), size: 25)),
          const SizedBox(height: 3),
  ...
  Widget _bottomGift(VoidCallback onTap) {
    return Padding(
      ...
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(30),
        child: Container(width: 58, height: 58, decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF9D2BFF), Color(0xFFFF3AA8)]), border: Border.all(color: Colors.white54, width: 1)), child: const Icon(Icons.card_giftcard_rounded, color: Colors.white, size: 28)),
      ),
    );
  }
  ...
  void _showGift(BuildContext context) {
    showModalBottomSheet<void>(
      ...
      showDragHandle: true,
      builder: (context) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 28), child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('Gifts', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)),
        const SizedBox(height: 14),
        const Text('Gift sending UI is ready. Connect your diamond/gift backend here.', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFFAAA6B6))),
        const SizedBox(height: 18),
        FilledButton.icon(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.card_giftcard_rounded), label: const Text('Close')),
      ]))),
    );
  }
  ...
  Future<void> _confirmClose(BuildContext context) async {
    ...
    if (close == true && context.mounted) {
      await zego.leave(context, showConfirmation: false);
    }
  }
  ...
  Future<void> _openGuestPicker(BuildContext context) async {
    ...
    await showModalBottomSheet<void>(
      ...
      showDragHandle: true,
      builder: (sheetContext) => SafeArea(child: StreamBuilder<List<ZegoUIKitUser>>(
        stream: controller.user.stream(includeFakeUser: false),
        builder: (context, snapshot) {
          final users = (snapshot.data ?? <ZegoUIKitUser>[]).where((u) => u.id != currentId).toList();
          return Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 18), child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Invite a guest', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)),
            const SizedBox(height: 12),
            if (users.isEmpty) const Padding(padding: EdgeInsets.all(24), child: Text('No viewers available yet.', style: TextStyle(color: Color(0xFFAAA6B6))))
            else Flexible(child: ListView.builder(shrinkWrap: true, itemCount: users.length, itemBuilder: (_, i) {
              final u = users[i];
              return ListTile(
                leading: CircleAvatar(backgroundColor: const Color(0xFF6D3D8C), child: Text(u.name.isEmpty ? '?' : u.name[0].toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900))),
                title: Text(u.name.isEmpty ? 'WikaLive User' : u.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
                trailing: FilledButton(onPressed: widget.isHost ? () async { await controller.coHost.hostSendCoHostInvitationToAudience(u, withToast: true); if (sheetContext.mounted) Navigator.pop(sheetContext); } : () async { await controller.coHost.audienceSendCoHostRequest(withToast: true); if (sheetContext.mounted) Navigator.pop(sheetContext); }, child: Text(widget.isHost ? 'Invite' : 'Join')),
              );
            })),
          ]));
        },
  ...
  Future<void> _openMoreTools(BuildContext context) async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(16, 6, 16, 24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _toolSection(String title, List<Widget> items) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: Color(0xFFBDB8CA), fontSize: 12, fontWeight: FontWeight.w900)), const SizedBox(height: 8), Wrap(spacing: 8, runSpacing: 8, children: items)]);
  ...
  Widget _sheetTool(IconData icon, String title, VoidCallback onTap) => InkWell(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(15),
        border: Border.all(color: const Color(0xFF302B3A)),
      ),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
