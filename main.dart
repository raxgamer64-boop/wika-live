  ...
  Widget build(BuildContext context) {
    ...
    return ZegoUIKitPrebuiltLiveStreaming(
      ...
      config: ZegoUIKitPrebuiltLiveStreamingConfig.audience(plugins: [ZegoUIKitSignalingPlugin()])
        ...
        ..foreground = const _GuestCallOverlay(isHost: false),
...
class _RealHostLivePageState extends State<RealHostLivePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final config = ZegoUIKitPrebuiltLiveStreamingConfig.host(plugins: [ZegoUIKitSignalingPlugin()])
      ...
      ..message = ZegoLiveStreamingInRoomMessageConfig(visible: false)
      ..foreground = const _GuestCallOverlay(isHost: true);
...
class _GuestCallOverlay extends StatefulWidget {
  final bool isHost;
  const _GuestCallOverlay({required this.isHost});
...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: [
      Positioned(
        ...
        child: SafeArea(
          ...
          child: StreamBuilder<List<ZegoUIKitUser>>(
            ...
            builder: (context, snapshot) {
              final all = (snapshot.data ?? <ZegoUIKitUser>[]).where((u) => u.id != localId).toList();
              final shown = all.take(3).toList();
              final count = all.length + 1;
              return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _hostChip(),
                const SizedBox(width: 8),
                Expanded(child: Center(child: _guardianAndUsers(shown))),
                const SizedBox(width: 8),
                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                  Row(mainAxisSize: MainAxisSize.min, children: [
                    _pill(Icons.person_rounded, '$count'),
                    const SizedBox(width: 7),
      ...
      Positioned(
        top: 92, left: 14,
        child: SafeArea(bottom: false, child: Row(mainAxisSize: MainAxisSize.min, children: [
          _smallPill(Icons.local_fire_department_rounded, 'Top 1'),
          const SizedBox(width: 6),
          _smallPill(Icons.diamond_rounded, '5.0K'),
        ])),
      ),
      if (widget.isHost)
        Positioned(right: 12, top: 48, child: SafeArea(bottom: false, child: _guestButton(context))),
      Positioned(
        left: 14, right: 92, bottom: 88,
        child: SafeArea(
          top: false,
          child: StreamBuilder<List<ZegoInRoomMessage>>(
            stream: controller.message.stream(includeFakeMessage: false),
            builder: (context, snapshot) {
              final messages = snapshot.data ?? <ZegoInRoomMessage>[];
              final recent = messages.length <= 4 ? messages : messages.sublist(messages.length - 4);
              if (recent.isEmpty) return const SizedBox.shrink();
              return Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start,
                children: recent.map((m) => _chatBubble(m.user.name, m.message)).toList());
            },
          ),
        ),
      ),
      Positioned(left: 14, right: 176, bottom: 16, child: SafeArea(top: false, child: _chatInput())),
      Positioned(
        right: 18, bottom: 130,
        child: SafeArea(top: false, child: IgnorePointer(child: Column(mainAxisSize: MainAxisSize.min, children: const [
          Icon(Icons.favorite_rounded, color: Color(0xFFFF3BBE), size: 30),
          SizedBox(height: 7),
          Icon(Icons.favorite_rounded, color: Color(0xFFFF45C7), size: 25),
          SizedBox(height: 7),
          Icon(Icons.favorite_rounded, color: Color(0xFFFF5AD1), size: 20),
        ]))),
      ),
      Positioned(
        right: 12, bottom: 12,
        child: SafeArea(top: false, child: Row(mainAxisSize: MainAxisSize.min, children: [
          _roundTool(Icons.more_horiz_rounded, 'More', () => _openMoreTools(context)),
          const SizedBox(width: 8),
          _roundTool(Icons.auto_fix_high_rounded, 'Beauty', () => ZegoUIKit().enableBeauty(true)),
          const SizedBox(width: 8),
          _roundTool(Icons.mic_rounded, 'Mic', () => controller.audioVideo.microphone.switchState()),
          const SizedBox(width: 8),
          _giftButton(context),
        ])),
      ),
  ...
  Widget _hostChip() => Container(
    ...
    decoration: BoxDecoration(color: const Color(0xD9101017), borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0x445F4A77))),
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      const CircleAvatar(radius: 19, backgroundColor: Color(0xFF5D2A82), child: Icon(Icons.person_rounded, color: Colors.white, size: 21)),
      const SizedBox(width: 7),
      const Flexible(child: Text('jerry ki duniya', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w900))),
      const SizedBox(width: 5),
      const Icon(Icons.shield_rounded, color: Color(0xFFFFC43D), size: 18),
    ]),
  );

  Widget _guardianAndUsers(List<ZegoUIKitUser> users) => Column(mainAxisSize: MainAxisSize.min, children: [
    Container(width: 58, height: 58, decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF8D35FF), Color(0xFF2D8DFF)]), boxShadow: const [BoxShadow(color: Color(0xAA8D35FF), blurRadius: 18)], border: Border.all(color: const Color(0xFFE7D7FF), width: 2)), child: const Icon(Icons.shield_rounded, color: Colors.white, size: 34)),
    const Text('GUARDIAN', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)),
    if (users.isNotEmpty) Row(mainAxisSize: MainAxisSize.min, children: users.map(_userMini).toList()),
  ]);

  Widget _userMini(ZegoUIKitUser u) {
    final label = u.name.trim().isEmpty ? '?' : u.name.trim()[0].toUpperCase();
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 2), child: Column(mainAxisSize: MainAxisSize.min, children: [
      CircleAvatar(radius: 15, backgroundColor: const Color(0xFF6E3F91), child: Text(label, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900))),
      SizedBox(width: 32, child: Text(u.name.isEmpty ? 'User' : u.name, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white70, fontSize: 6.5, fontWeight: FontWeight.w700))),
    ]));
  }

  Widget _pill(IconData icon, String text) => Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), decoration: BoxDecoration(color: const Color(0xCC101017), borderRadius: BorderRadius.circular(20)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: Colors.white, size: 17), const SizedBox(width: 5), Text(text, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900))]));
  Widget _smallPill(IconData icon, String text) => Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7), decoration: BoxDecoration(color: const Color(0xCC101017), borderRadius: BorderRadius.circular(18)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: const Color(0xFFFFD23F), size: 15), const SizedBox(width: 4), Text(text, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900))]));
  Widget _gemPill(String text) => Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 8), decoration: BoxDecoration(color: const Color(0xCC101017), borderRadius: BorderRadius.circular(20)), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.diamond_rounded, color: Color(0xFFE75CFF), size: 17), const SizedBox(width: 5), Text(text, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900)), const SizedBox(width: 5), const Icon(Icons.chevron_right_rounded, color: Colors.white70, size: 17)]));

  Widget _closeButton(BuildContext context) => InkWell(onTap: () => ZegoUIKitPrebuiltLiveStreamingController().leave(context, showConfirmation: true), borderRadius: BorderRadius.circular(22), child: Container(width: 42, height: 42, decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0xCC101017)), child: const Icon(Icons.close_rounded, color: Colors.white, size: 25)));

  Widget _guestButton(BuildContext context) => InkWell(onTap: () => _openGuestPicker(context), borderRadius: BorderRadius.circular(18), child: Container(width: 76, height: 76, decoration: BoxDecoration(color: const Color(0x33101017), borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xBFFFFFFF), width: 1.2)), child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.add_rounded, color: Colors.white, size: 32), Text('Guest', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800))])));

  Widget _chatBubble(String name, String message) => Padding(padding: const EdgeInsets.only(bottom: 5), child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7), decoration: BoxDecoration(color: const Color(0xB8101017), borderRadius: BorderRadius.circular(17)), child: RichText(text: TextSpan(children: [TextSpan(text: '${name.isEmpty ? 'User' : name}: ', style: const TextStyle(color: Color(0xFFFF8BDE), fontSize: 11, fontWeight: FontWeight.w900)), TextSpan(text: message, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600))]))));

  Widget _chatInput() => Container(height: 46, decoration: BoxDecoration(color: const Color(0xCC101017), borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0x334A4557))), child: Row(children: [
    Expanded(child: TextField(controller: _chatController, style: const TextStyle(color: Colors.white, fontSize: 12), decoration: const InputDecoration(hintText: 'Say something...', hintStyle: TextStyle(color: Colors.white60, fontSize: 12), border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 15)), onSubmitted: (_) => _sendMessage())),
    IconButton(onPressed: _sendMessage, icon: const Icon(Icons.sentiment_satisfied_alt_rounded, color: Colors.white, size: 22)),
  ]));

  Future<void> _sendMessage() async { final value = _chatController.text.trim(); if (value.isEmpty) return; _chatController.clear(); await ZegoUIKitPrebuiltLiveStreamingController().message.send(value); }

  Widget _roundTool(IconData icon, String label, VoidCallback onTap) => Column(mainAxisSize: MainAxisSize.min, children: [InkWell(onTap: onTap, borderRadius: BorderRadius.circular(30), child: Container(width: 58, height: 58, decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0xD9101017)), child: Icon(icon, color: Colors.white, size: 25))), const SizedBox(height: 3), Text(label, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800))]);

  Widget _giftButton(BuildContext context) => Column(mainAxisSize: MainAxisSize.min, children: [InkWell(onTap: () => msg(context, 'Gifts will be connected next.'), borderRadius: BorderRadius.circular(30), child: Container(width: 60, height: 60, decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [Color(0xFFF72CC5), Color(0xFF6A2CFF)])), child: const Icon(Icons.card_giftcard_rounded, color: Colors.white, size: 27))), const SizedBox(height: 3), const Text('Gift', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800))]);

  Future<void> _openGuestPicker(BuildContext context) async {
    final controller = ZegoUIKitPrebuiltLiveStreamingController();
    final currentId = FirebaseAuth.instance.currentUser?.uid;
    await showModalBottomSheet<void>(context: context, backgroundColor: const Color(0xFF11101A), showDragHandle: true, builder: (sheetContext) => SafeArea(child: StreamBuilder<List<ZegoUIKitUser>>(
      stream: controller.user.stream(includeFakeUser: false),
      builder: (context, snapshot) {
        final users = (snapshot.data ?? <ZegoUIKitUser>[]).where((u) => u.id != currentId).toList();
        return Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 18), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Invite a guest', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)),
          const SizedBox(height: 6),
          const Text('Choose someone currently in this live room.', style: TextStyle(color: Color(0xFF9892A5), fontSize: 12)),
          const SizedBox(height: 12),
          if (users.isEmpty) const Padding(padding: EdgeInsets.symmetric(vertical: 28), child: Center(child: Text('No viewers available yet.', style: TextStyle(color: Color(0xFF9C95A8)))))
          else Flexible(child: ListView.separated(shrinkWrap: true, itemCount: users.length, separatorBuilder: (_, __) => const Divider(color: Color(0xFF282332)), itemBuilder: (_, i) { final u = users[i]; return ListTile(
            leading: CircleAvatar(backgroundColor: const Color(0xFF6D3D8C), child: Text(u.name.isNotEmpty ? u.name[0].toUpperCase() : '?', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900))),
            title: Text(u.name.isEmpty ? 'WikaLive User' : u.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
            trailing: FilledButton(onPressed: () async { final ok = await controller.coHost.hostSendCoHostInvitationToAudience(u, withToast: true); if (sheetContext.mounted) Navigator.of(sheetContext).pop(); if (!ok && context.mounted) msg(context, 'Guest invitation could not be sent.'); }, child: const Text('Invite')),
          ); })),
        ]));
      },
    )));
  ...
  Future<void> _openMoreTools(BuildContext context) async {
    final controller = ZegoUIKitPrebuiltLiveStreamingController();
    await showModalBottomSheet<void>(context: context, backgroundColor: const Color(0xFF0F0E16), showDragHandle: true, isScrollControlled: true, builder: (sheetContext) => SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(16, 6, 16, 24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _toolSection('Room tools', [
        ...
        _sheetTool(Icons.face_retouching_natural_rounded, 'Beauty', () => ZegoUIKit().enableBeauty(true)),
      ...
      _toolSection('Basic tools', [
        ...
        _sheetTool(Icons.mic_rounded, 'Microphone', () => controller.audioVideo.microphone.switchState()),
        _sheetTool(Icons.flip_camera_android_rounded, 'Mirror', () => msg(sheetContext, 'Mirror control is ready.')),
        _sheetTool(Icons.tune_rounded, 'Effect settings', () => msg(sheetContext, 'Effect settings are ready.')),
      ]),
    ]))));
  }

  Widget _toolSection(String title, List<Widget> tools) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900)), const SizedBox(height: 10), GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 4, mainAxisSpacing: 12, crossAxisSpacing: 10, childAspectRatio: .95, children: tools)]);
  Widget _sheetTool(IconData icon, String label, VoidCallback onTap) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(17), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 54, height: 54, decoration: BoxDecoration(color: const Color(0xFF181622), borderRadius: BorderRadius.circular(17), border: Border.all(color: const Color(0x332F2840))), child: Icon(icon, color: const Color(0xFFE78CFF), size: 25)), const SizedBox(height: 6), Text(label, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800))]));
}
