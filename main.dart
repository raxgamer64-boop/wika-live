...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
    ...
    return StreamBuilder<List<ZegoUIKitUser>>(
      ...
      builder: (context, roomSnapshot) {
        ...
        return Stack(
          fit: StackFit.expand,
          children: [
            // ZEGO video stays underneath. The reference interface is an edge-to-edge overlay.
            ...
            Positioned(
              ...
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(child: _referenceHostIdentity()),
                  const SizedBox(width: 8),
                  _referenceRoundButton(Icons.add_rounded, 58, _openMoreTools),
                  const SizedBox(width: 8),
                  _referenceGuardian(),
                  const SizedBox(width: 8),
                  Flexible(flex: 0, child: _referenceTopUsers()),
                  const SizedBox(width: 8),
                  _referenceViewerCount(audienceCount),
                  const SizedBox(width: 7),
                  _referenceRoundButton(Icons.close_rounded, 52, _confirmClose),
                ],
              ),
            ),

            // Room shortcuts directly below the header.
            Positioned(
              top: top + 104,
              left: 20,
              child: Row(
                children: [
                  _referencePill(Icons.calendar_month_rounded, '', 48),
                  const SizedBox(width: 9),
                  _referencePill(Icons.star_rounded, 'Lucky Room', 185, trailing: Icons.chevron_right_rounded),
                  const SizedBox(width: 10),
                  _referencePill(Icons.local_fire_department_rounded, '33.1k', 132, trailing: Icons.chevron_right_rounded),
                ],
              ),
            ),
            ...
            Positioned(
              ...
              left: 0,
              right: 0,
              child: _referenceAnnouncement(),
            ),

            // PK score strip. Game functionality is NOT wired to the game UI; this is only the reference room visual.
            Positioned(
              top: top + 235,
              left: 0,
              right: 0,
              child: _referencePkScore(),
            ),

            // Guest/co-host video remains controlled by ZEGO; small room indicators match the reference position.
            Positioned(
              top: top + 278,
              right: 18,
              child: _referenceMicLabel(guests.isNotEmpty ? guests.first.name : 'afsraa'),
            ),

            // Three host-side and three guest-side seats at the lower edge of the video.
            Positioned(
              left: 18,
              right: 18,
              ...
              child: Row(
                children: [
                  _referenceSeats(Color(0xFFFF3EA9)),
                  const Spacer(),
                  _referenceSeats(Color(0xFF318DFF)),
                ],
              ),
            ),
            ...
            Positioned(
              left: 0,
              right: 0,
              top: MediaQuery.sizeOf(context).height * .545,
              child: Center(child: _referenceRoomLabel()),
            ),

            // Win/gift announcement.
            Positioned(
              left: 26,
              top: MediaQuery.sizeOf(context).height * .595,
              child: _referenceWinBanner(),
            ),

            // Real ZEGO chat messages, styled to the reference.
            Positioned(
              left: 28,
              right: 245,
              bottom: bottom + 170,
              child: StreamBuilder<List<ZegoInRoomMessage>>(
                ...
                builder: (context, messageSnapshot) {
                  ...
                  final visible = messages.length > 5 ? messages.sublist(messages.length - 5) : messages;
                  if (visible.isEmpty) {
                  ...
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisSize: MainAxisSize.min,
                    children: visible.map(_referenceMessage).toList(),
                  );
                },
              ),
            ),
                ...
                child: SizedBox(
                  ...
                  child: Stack(
                    ...
                    children: List.generate(hearts, (index) {
                      final offsets = <double>[0, 38, 80, 124, 168, 205, 225];
                      final sizes = <double>[36, 31, 42, 34, 39, 33, 37];
                      return Positioned(
                        bottom: offsets[index.clamp(0, offsets.length - 1)],
                        right: index.isEven ? 0 : 10,
                        child: Icon(
                          Icons.favorite_rounded,
                          color: const Color(0xFFFF3FB7),
                          size: sizes[index.clamp(0, sizes.length - 1)],
                          shadows: const [Shadow(color: Color(0xFF9D26FF), blurRadius: 14)],
                        ),
                      );
                    }),
                  ),
            ...
            // Exact bottom composition: input + three circular actions.
            Positioned(
              left: 28,
              right: 28,
              bottom: bottom + 22,
              child: Row(
                ...
                children: [
                  Expanded(child: _referenceChatInput()),
                  const SizedBox(width: 14),
                  _referenceCircleAction(Icons.more_horiz_rounded, 64, () => _openMoreTools(context)),
                  const SizedBox(width: 12),
                  _referenceCircleAction(Icons.local_florist_rounded, 64, () => _showReceivedGifts(context), badge: '90% OFF'),
                  const SizedBox(width: 12),
                  _referenceGiftButton(() => _showGift(context)),
                ],
              ),
            ),
          ],
  ...
  Widget _referenceHostIdentity() {
    final uid = _resolvedHostUid;
    if (uid == null) {
      return _referenceHostIdentityContent(widget.hostName, widget.hostPhotoUrl ?? '', 32);
    }
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: _userStream(uid),
      builder: (context, snapshot) {
        final d = snapshot.data?.data() ?? <String, dynamic>{};
        return _referenceHostIdentityContent(
          (d['name'] ?? widget.hostName).toString(),
          _photo(d, widget.hostPhotoUrl),
          d['level'] is num ? (d['level'] as num).toInt() : 32,
        );
      },
    );
  }

  Widget _referenceHostIdentityContent(String name, String photo, int level) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Container(
          padding: const EdgeInsets.all(2),
          decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFFFF55B8), width: 2)),
          child: _avatar(url: photo, name: name, radius: 29),
        ),
        const SizedBox(width: 8),
        Flexible(
          child: Column(
            ...
            children: [
              Row(children: [
                Flexible(child: Text('❤️ $name 👑', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900))),
              ]),
              const SizedBox(height: 4),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 3),
                decoration: BoxDecoration(color: const Color(0xFF7944F5), borderRadius: BorderRadius.circular(12)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.workspace_premium_rounded, color: Colors.white, size: 12), const SizedBox(width: 4), Text('Lv.$level', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900))]),
              ),
              const SizedBox(height: 4),
              Row(children: [const Icon(Icons.diamond_rounded, color: Color(0xFFFF4BD0), size: 19), const SizedBox(width: 5), const Text('7,770', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900))]),
            ],
          ),
        ),
      ],
  ...
  Widget _referenceRoundButton(IconData icon, double size, void Function(BuildContext) action) => InkWell(
        onTap: () => action(context),
        borderRadius: BorderRadius.circular(size),
        child: Container(
          width: size,
          height: size,
          decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFFFF4CCB), Color(0xFF8A32FF)]), border: Border.all(color: Colors.white.withOpacity(.35)), boxShadow: const [BoxShadow(color: Color(0x774E19FF), blurRadius: 18)]),
          child: Icon(icon, color: Colors.white, size: size * .53),
        ),
      );

  Widget _referenceGuardian() => Container(
        width: 76,
        height: 76,
        decoration: BoxDecoration(shape: BoxShape.circle, gradient: const RadialGradient(colors: [Color(0xFFD681FF), Color(0xFF5520B6), Color(0xFF130B31)]), border: Border.all(color: const Color(0xFFFFB7FF), width: 2), boxShadow: const [BoxShadow(color: Color(0xAA8A2FFF), blurRadius: 20)]),
        child: ClipOval(child: Image.asset('assets/live_ui/ic_guardian_gold.png', fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Center(child: Text('W', style: TextStyle(color: Colors.white, fontSize: 42, fontWeight: FontWeight.w900))))),
      );

  Widget _referenceTopUsers() => Row(mainAxisSize: MainAxisSize.min, children: [
        _referenceMiniUser(Icons.workspace_premium_rounded, 'WIKA', const Color(0xFFFFD34D)),
        _referenceMiniUser(Icons.person_rounded, '', const Color(0xFF61BFFF)),
        _referenceMiniUser(Icons.person_rounded, '', const Color(0xFFFF9E68)),
      ]);

  Widget _referenceMiniUser(IconData icon, String label, Color ring) => Container(
        margin: const EdgeInsets.only(left: 4),
        width: 52,
        height: 52,
        decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xCC17152B), border: Border.all(color: ring, width: 2)),
        child: Center(child: label.isNotEmpty ? Text(label, style: TextStyle(color: ring, fontSize: 9, fontWeight: FontWeight.w900)) : Icon(icon, color: Colors.white, size: 25)),
      );

  Widget _referenceViewerCount(int count) => Container(
        height: 50,
        padding: const EdgeInsets.symmetric(horizontal: 12),
        decoration: BoxDecoration(color: const Color(0x99100F20), borderRadius: BorderRadius.circular(25), border: Border.all(color: const Color(0x663F4B82))),
        child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.person_rounded, color: Colors.white, size: 21), const SizedBox(width: 5), Text('$count', style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w900))]),
      );

  Widget _referencePill(IconData icon, String text, double width, {IconData? trailing}) => Container(
        width: width,
        height: 48,
        padding: const EdgeInsets.symmetric(horizontal: 13),
        decoration: BoxDecoration(color: const Color(0x6610122C), borderRadius: BorderRadius.circular(25), border: Border.all(color: const Color(0x664E4D9D))),
        child: Row(children: [Icon(icon, color: icon == Icons.local_fire_department_rounded ? const Color(0xFFFF742F) : const Color(0xFFFFD34D), size: 25), if (text.isNotEmpty) ...[const SizedBox(width: 8), Expanded(child: Text(text, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w900)))], if (trailing != null) Icon(trailing, color: Colors.white, size: 22)]),
      );

  Widget _referenceAnnouncement() => Container(
        height: 67,
        decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFFFA80D), Color(0xFFFF6C16), Color(0xFFFFB41C)])),
        child: Row(children: [
          const SizedBox(width: 18),
          Container(width: 54, height: 54, decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0x663A2100)), child: const Icon(Icons.emoji_events_rounded, color: Color(0xFFFFE79A), size: 33)),
          const SizedBox(width: 12),
          const Icon(Icons.emoji_events_rounded, color: Colors.white, size: 27),
          const SizedBox(width: 8),
          const Expanded(child: Text('JERRY is winning  🪙 100000 in WikaLive', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900))),
          const Padding(padding: EdgeInsets.only(right: 16), child: Icon(Icons.card_giftcard_rounded, color: Colors.white, size: 45)),
        ]),
      );

  Widget _referencePkScore() => SizedBox(
        height: 43,
        child: Row(children: [
          Expanded(child: Container(alignment: Alignment.centerLeft, padding: const EdgeInsets.only(left: 22), decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFFF147A), Color(0xFFFF4FA7)])), child: const Text('5200', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)))),
          Container(width: 130, height: 43, alignment: Alignment.center, decoration: const BoxDecoration(color: Color(0xFF2D234F), borderRadius: BorderRadius.vertical(bottom: Radius.circular(25))), child: const Text('PK  00:54', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900))),
          Expanded(child: Container(alignment: Alignment.centerRight, padding: const EdgeInsets.only(right: 22), decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF22BFF8), Color(0xFF12A7EE)])), child: const Text('4300', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)))),
        ]),
      );

  Widget _referenceMicLabel(String name) => Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8), decoration: BoxDecoration(color: const Color(0xCC6D6F78), borderRadius: BorderRadius.circular(20)), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.mic_rounded, color: Colors.white, size: 19), const SizedBox(width: 6), Text(name.isEmpty ? 'afsraa' : name, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w800))]));

  Widget _referenceSeats(Color color) => Row(children: List.generate(3, (i) => Container(margin: const EdgeInsets.only(right: 9), width: 58, height: 58, decoration: BoxDecoration(shape: BoxShape.circle, color: color.withOpacity(.9), border: Border.all(color: Colors.white.withOpacity(.3), width: 1.5), boxShadow: [BoxShadow(color: color.withOpacity(.45), blurRadius: 14)]), padding: const EdgeInsets.all(8), child: Image.asset(color.value == const Color(0xFFFF3EA9).value ? 'assets/live_ui/iv_live_team_pk_red_seat.png' : 'assets/live_ui/iv_live_team_pk_blue_seat.png', fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.event_seat_rounded, color: Colors.white, size: 27)))));

  Widget _referenceRoomLabel() => Container(padding: const EdgeInsets.symmetric(horizontal: 42, vertical: 13), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF8D25D9), Color(0xFF5A31D7)]), borderRadius: BorderRadius.circular(25), border: Border.all(color: const Color(0xFFFF4DD7), width: 1.2), boxShadow: const [BoxShadow(color: Color(0xAA7B2DFF), blurRadius: 16)]), child: const Text('WikaLive PK Room', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900)));

  Widget _referenceWinBanner() => Container(padding: const EdgeInsets.fromLTRB(8, 8, 18, 8), decoration: BoxDecoration(color: const Color(0xDD241820), borderRadius: BorderRadius.circular(28), border: Border.all(color: const Color(0xFFFFB238), width: 1.5), boxShadow: const [BoxShadow(color: Color(0x552C0F00), blurRadius: 12)]), child: Row(mainAxisSize: MainAxisSize.min, children: [const CircleAvatar(radius: 25, backgroundColor: Color(0xFF30253D), child: Icon(Icons.person_rounded, color: Colors.white)), const SizedBox(width: 9), const Text('Win 🪙2000', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(width: 8), const Icon(Icons.local_florist_rounded, color: Color(0xFF6DFF45), size: 42)]));

  Widget _referenceMessage(ZegoInRoomMessage message) => Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 9), decoration: BoxDecoration(color: const Color(0xD70C0C15), borderRadius: BorderRadius.circular(17)), child: RichText(text: TextSpan(children: [const TextSpan(text: '🟢 3  ', style: TextStyle(color: Color(0xFF9CFF43), fontSize: 14, fontWeight: FontWeight.w900)), TextSpan(text: '${message.user.name.isEmpty ? 'User' : message.user.name}: ', style: const TextStyle(color: Color(0xFF59D5FF), fontSize: 14, fontWeight: FontWeight.w900)), TextSpan(text: message.message, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w700))])));

  Widget _referencePromo(IconData icon, String label) => Container(width: 118, height: 110, decoration: BoxDecoration(borderRadius: BorderRadius.circular(30), gradient: const RadialGradient(colors: [Color(0xFFB94BFF), Color(0xFF2B0E57), Color(0x00000000)]), boxShadow: const [BoxShadow(color: Color(0x884D00FF), blurRadius: 25)]), child: Image.asset(label.contains('WHEEL') ? 'assets/live_ui/iv_lucky_button.png' : 'assets/live_ui/bg_treasure_box.png', fit: BoxFit.contain, errorBuilder: (_, __, ___) => Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: const Color(0xFFFFD45A), size: 37), Text(label, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w900))])));

  Widget _referenceChatInput() => Container(height: 64, decoration: BoxDecoration(color: const Color(0xD70C0D19), borderRadius: BorderRadius.circular(34), border: Border.all(color: const Color(0xAA8D78FF), width: 1.4)), child: Row(children: [const SizedBox(width: 18), const Icon(Icons.send_rounded, color: Colors.white, size: 27), Expanded(child: TextField(controller: _chatController, onSubmitted: (_) => _sendMessage(), style: const TextStyle(color: Colors.white, fontSize: 16), decoration: const InputDecoration(hintText: 'Hi~', hintStyle: TextStyle(color: Color(0xFFB7B3C4), fontSize: 16), border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 15)))), InkWell(onTap: _sendHeart, child: const Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Icon(Icons.emoji_emotions_outlined, color: Colors.white, size: 30)))]));

  Widget _referenceCircleAction(IconData icon, double size, VoidCallback onTap, {String? badge}) => SizedBox(width: size, height: size + (badge == null ? 0 : 18), child: Stack(clipBehavior: Clip.none, children: [InkWell(onTap: onTap, borderRadius: BorderRadius.circular(size), child: Container(width: size, height: size, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xAA17142B), border: Border.all(color: const Color(0xFFFF42C2), width: 1.4), boxShadow: const [BoxShadow(color: Color(0x663F16FF), blurRadius: 15)]), child: Icon(icon, color: Colors.white, size: 30))), if (badge != null) Positioned(bottom: -3, left: -4, right: -4, child: Container(padding: const EdgeInsets.symmetric(vertical: 3), decoration: BoxDecoration(color: const Color(0xFFFF3DAF), borderRadius: BorderRadius.circular(10)), child: Text(badge, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900))))]));

  Widget _referenceGiftButton(VoidCallback onTap) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(34), child: Container(width: 70, height: 70, decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFFB12EFF), Color(0xFF3C83FF)]), border: Border.all(color: Colors.white.withOpacity(.45), width: 1.5), boxShadow: const [BoxShadow(color: Color(0xAA9228FF), blurRadius: 22)]), child: Image.asset('assets/live_ui/iv_gift_btn.png', fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.card_giftcard_rounded, color: Colors.white, size: 38))));
  ...
  Widget _sideAction(IconData icon, String label, VoidCallback onTap, {bool active = true}) {
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(24),
      child: Column(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              shape: BoxShape.circle,
              border: Border.all(
                color: active ? Colors.white.withOpacity(.18) : const Color(0xFFFF55B8),
              ),
            ),
            child: Icon(icon, color: active ? Colors.white : const Color(0xFFFF55B8), size: 20),
          ),
          const SizedBox(height: 3),
  ...
  Future<void> _showReceivedGifts(BuildContext context) async {
    final uid = _resolvedHostUid ?? FirebaseAuth.instance.currentUser?.uid;
      ...
      builder: (sheetContext) => SafeArea(
        child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            final diamonds = data['liveDiamonds'] is num ? (data['liveDiamonds'] as num).toInt() : 0;
            final gifts = data['liveGiftCount'] is num ? (data['liveGiftCount'] as num).toInt() : 0;
            return Padding(
              ...
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Live Gifts', style: TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 14),
                  Row(
                    children: [
                      Expanded(child: _glass(padding: const EdgeInsets.all(14), radius: 18, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.diamond_rounded, color: Color(0xFF7ED8FF)), const SizedBox(height: 6), Text(_shortNumber(diamonds), style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)), const Text('Diamonds', style: TextStyle(color: Colors.white60, fontSize: 11))]))),
                      const SizedBox(width: 10),
                      Expanded(child: _glass(padding: const EdgeInsets.all(14), radius: 18, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.card_giftcard_rounded, color: Color(0xFFFF7BD1)), const SizedBox(height: 6), Text(_shortNumber(gifts), style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)), const Text('Gifts', style: TextStyle(color: Colors.white60, fontSize: 11))]))),
                    ],
                  ),
                ],
              ),
  ...
  Widget _compactTopHeader(int audienceCount) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final width = constraints.maxWidth;
        final narrow = width < 380;
        final closeSize = narrow ? 40.0 : 44.0;
        final viewerWidth = narrow ? 54.0 : 62.0;
        final guardianSize = narrow ? 40.0 : 44.0;
        ...
        return Row(
          ...
          children: [
            ...
            _guardianBadgeSmall(guardianSize),
            SizedBox(width: narrow ? 4 : 7),
            Expanded(
              flex: 3,
              child: Align(
                alignment: Alignment.centerLeft,
                child: ClipRect(child: _rankingUsersOnly()),
              ),
            ),
            SizedBox(width: narrow ? 4 : 7),
            SizedBox(
              ...
              child: _glass(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.person_rounded, color: Colors.white, size: narrow ? 15 : 17),
            ...
            _roundGlass(Icons.close_rounded, closeSize, () => _confirmClose(context)),
          ],
        );
      },
    );
          ...
          Stack(
            ...
            children: [
              ...
              Container(
                ...
                child: _avatar(url: photo, name: name, radius: 16),
              ),
            ],
          ),
          const SizedBox(height: 3),
  ...
  Widget _guardianBadgeSmall(double size) {
    return SizedBox(
      width: size,
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: const RadialGradient(
                colors: [Color(0xFFB96CFF), Color(0xFF3C16A0), Color(0xFF100827)],
              ),
              border: Border.all(color: const Color(0xFFD48BFF), width: 1.5),
              boxShadow: const [
                BoxShadow(color: Color(0xCC8E3CFF), blurRadius: 12, spreadRadius: 1),
              ],
            ),
            child: Icon(Icons.shield_rounded, color: Colors.white, size: size * .55),
          ),
          ...
          const Text(
            ...
            style: TextStyle(
              ...
              fontSize: 7,
              fontWeight: FontWeight.w900,
              shadows: [Shadow(color: Color(0xFF9C35FF), blurRadius: 7)],
            ),
          ),
        ],
      ),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.add_rounded, color: Colors.white, size: compact ? 24 : 30),
            const SizedBox(height: 1),
            Text(
              'Guest',
              style: TextStyle(color: Colors.white, fontSize: compact ? 8 : 11, fontWeight: FontWeight.w800),
            ),
          ],
        ),
      ),
  ...
  Widget _chatBubble(String name, String message, String photo, int level) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _avatar(url: photo, name: name, radius: 21),
          const SizedBox(width: 8),
          Flexible(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text('Lv.$level', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900)),
                        ],
                      ),
                    ),
                  ],
  ...
  Widget _welcomePill() => const SizedBox.shrink();
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 3),
            const Text('Gift', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800)),
          ],
        ),
      ),
    );
