...
class PartyScreen extends StatelessWidget {
  ...
  Widget _filterRow() => SingleChildScrollView(
        scrollDirection: Axis.horizontal,
        child: Row(children: ['All', 'Hot', 'Music', 'Chat', 'PK Battle'].map((x) => Container(margin: const EdgeInsets.only(right: 8), padding: const EdgeInsets.symmetric(horizontal: 17, vertical: 10), decoration: BoxDecoration(color: x == 'All' ? const Color(0xFFE72BA1) : const Color(0xFF111629), borderRadius: BorderRadius.circular(22), border: Border.all(color: x == 'All' ? const Color(0x00FFFFFF) : const Color(0x223E486A))), child: Text(x, style: TextStyle(color: x == 'All' ? Colors.white : const Color(0xFFA7AAC0), fontWeight: FontWeight.w800)))).toList()),
      );
...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
    ...
    return StreamBuilder<List<ZegoUIKitUser>>(
      ...
      builder: (context, roomSnapshot) {
        ...
        return Stack(
          ...
          children: [
            // =============================================================
            // Z-INDEX 1 — TOP LIVE-ROOM HEADER
            // Host identity + Guardian + Share/Gift/More + viewers + close.
            // =============================================================
            Positioned(
              top: top + 10,
              left: 10,
            ...
            // =============================================================
            // Z-INDEX 2 — HORIZONTAL SUPPORTER / RANKING STRIP
            // Avatar + amount gifted; horizontally scrollable.
            // =============================================================
            Positioned(
              top: top + 83,
              left: 12,
            ...
            // =============================================================
            // Z-INDEX 2 — ANNOUNCEMENT OVER VIDEO
            // =============================================================
            Positioned(
              top: top + 140,
              left: 12,
              right: 12,
              child: _announcementBanners(),
            ),

            // =============================================================
            // Z-INDEX 2 — LEFT CAMERA / MIC / BEAUTY CONTROLS
            // =============================================================
            Positioned(
              left: 10,
              top: media.size.height * .40,
              child: _leftRoomControls(),
            ),

            // =============================================================
            // Z-INDEX 2 — GUEST / CO-HOST SLOTS
            // =============================================================
            Positioned(
              right: 10,
              top: media.size.height * .42,
              child: _guestArea(context, guests),
            ),

            // =============================================================
            // Z-INDEX 2 — LIVE INCOME / WARNING / CHAT
            // =============================================================
            Positioned(
              left: 12,
              bottom: bottom + 280,
              ...
              child: StreamBuilder<List<ZegoInRoomMessage>>(
                ...
                builder: (context, messageSnapshot) {
                  ...
                  return ConstrainedBox(
                    constraints: BoxConstraints(maxHeight: media.size.height * .25),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      mainAxisSize: MainAxisSize.min,
            ...
            // Floating hearts remain part of the live interaction UI.
            Positioned(
              right: 8,
              bottom: bottom + 180,
              child: IgnorePointer(
                child: SizedBox(
                  ...
                  child: Stack(
                    ...
                    children: List.generate(hearts, (index) {
                      const offsets = <double>[0, 48, 98, 150, 205, 255, 285];
                      const sizes = <double>[43, 39, 46, 37, 44, 40, 42];
                      return Positioned(
            ...
            // Bottom readability gradient over the live video.
            Positioned(
              left: 0,
              right: 0,
              bottom: 0,
                ...
                child: DecoratedBox(
                  decoration: const BoxDecoration(
                    gradient: LinearGradient(
                      ...
                      colors: [Colors.transparent, Color(0x55000000), Color(0xF5000000)],
                      stops: [0.0, .42, 1.0],
                    ),
                  ),
                ),
            ...
            // =============================================================
            // Z-INDEX 3 — BOTTOM CHAT / ACTION BAR
            // =============================================================
            Positioned(
              left: 12,
              bottom: bottom + 12,
              right: media.size.width * .34,
              child: _chatInput(),
            ),
            Positioned(
              right: 12,
              bottom: bottom + 10,
              child: _bottomMailMenu(),
            ),
            Positioned(
              right: 8,
              bottom: bottom + 72,
              child: _bottomGiftWithTimer(() => _showGift(context)),
            ),
          ],
        );
      },
      ...
      children: [
        _glass(
          ...
          child: const Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.local_fire_department_rounded, color: Color(0xFFFF6A3D), size: 21),
              SizedBox(width: 5),
              Text('Lucky Ranking', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900)),
              SizedBox(width: 7),
              Text('19.4k', style: TextStyle(color: Color(0xFFFFD34D), fontSize: 12, fontWeight: FontWeight.w900)),
            ],
          ),
        ),
        const SizedBox(width: 7),
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          return _rankDataAvatar(
                            index + 1,
                            (d['name'] ?? 'User').toString(),
                            (d['photoUrl'] ?? '').toString(),
                            d['diamondsSent'] is num ? (d['diamondsSent'] as num).toInt() : 0,
                          );
                        },
                      ),
                    );
  ...
  Widget _leftRoomControls() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        _roundGlass(Icons.flip_camera_android_rounded, 44, () => msg(context, 'Camera switch control')),
        const SizedBox(height: 9),
        _roundGlass(micOn ? Icons.mic_rounded : Icons.mic_off_rounded, 44, _toggleMic),
        const SizedBox(height: 9),
        _roundGlass(
          beautyOn ? Icons.face_retouching_natural_rounded : Icons.face_rounded,
          44,
          _toggleBeauty,
        ),
      ],
  ...
  Widget _announcementBanners() {
    return _announcement(
      ...
      text: 'Congratulations to the top supporter for sending a gift!',
  ...
  Widget _compactTopHeader(int audienceCount) {
    return Row(
      ...
      children: [
        Expanded(child: _hostCard()),
        const SizedBox(width: 7),
        _guardianBadgeSmall(48),
        const Spacer(),
        _roundGlass(Icons.share_outlined, 40, () {
          Clipboard.setData(ClipboardData(text: 'WikaLive live room: ${widget.liveId}'));
          msg(context, 'Live room link copied');
        }),
        const SizedBox(width: 5),
        _roundGlass(Icons.card_giftcard_rounded, 40, () => _showGift(context)),
        const SizedBox(width: 5),
        _roundGlass(Icons.more_horiz_rounded, 40, () => _openMoreTools(context)),
        const SizedBox(width: 5),
        _glass(
        ...
        const SizedBox(width: 5),
        _roundGlass(Icons.close_rounded, 40, () => _confirmClose(context)),
      ],
    );
  }
          ...
          Stack(
            ...
            children: [
              Container(
                ...
                child: _avatar(url: photo, name: name, radius: 17),
              ),
              Positioned(
                top: -1,
                left: 0,
