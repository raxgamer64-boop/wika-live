...
class _AuthPageState extends State<AuthPage> {
  ...
  Future<void> submit() async {
    ...
    // New account passwords must contain both letters and numbers.
    // Require at least 8 characters for a stronger account password.
    if (!login) {
      ...
      if (password.length < 8 || !hasLetter || !hasNumber) {
        msg(
          context,
          'Password must be 8+ characters and contain letters + numbers',
        );
        return;
      }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if (!login)
                const Padding(
                  padding: EdgeInsets.only(left: 6, top: 0, bottom: 8),
                  child: Text(
                    'Use 8+ characters with at least 1 letter and 1 number.',
                    style: TextStyle(
                      color: Color(0xFF77727F),
                      fontSize: 12,
  ...
  Widget _liveVideoBackground(String? url) {
    if (url != null && url.isNotEmpty) {
      return Image.network(
        url,
        fit: BoxFit.cover,
        errorBuilder: (_, __, ___) => _placeholderVideo(),
      );
    }
    return _placeholderVideo();
  }

  Widget _placeholderVideo() {
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment.topCenter,
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: RadialGradient(
                colors: [
                  const Color(0xFFB044FF).withValues(alpha: .24),
                  Colors.transparent,
                ],
              ),
            ),
