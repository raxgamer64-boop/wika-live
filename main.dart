...
class GameCenterPage extends StatelessWidget {
        ...
        builder: (context, snapshot) {
          ...
          return ListView(
            ...
            children: [
              _gameCoinCard(coins),
              const SizedBox(height: 16),
              const Text('MINI GAMES', style: TextStyle(color: Color(0xFFFFD76A), fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1.4)),
              const SizedBox(height: 10),
              _gameTile(context, 'Lucky Spin', 'Spin the wheel and win virtual Coins', '🎡', const Color(0xFFE94FA8), 50, const LuckySpinPage()),
              _gameTile(context, 'Roulette Wheel', 'Pick a color/number and test your luck', '🎰', const Color(0xFF9B59FF), 100, const RoulettePage()),
              _gameTile(context, 'Greedy Chest', 'Open a chest for a surprise Coin reward', '💎', const Color(0xFF16B9C7), 100, const GreedyChestPage()),
              _gameTile(context, 'Lucky Box', 'Choose one of three mystery boxes', '🎁', const Color(0xFFFF9F43), 200, const LuckyBoxPage()),
              const SizedBox(height: 10),
              Container(
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(color: const Color(0xFF1D1628), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0x33FFFFFF))),
                child: const Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Icon(Icons.info_outline_rounded, color: Color(0xFFFFD76A), size: 20),
                  SizedBox(width: 10),
                  Expanded(child: Text('Games use WikaLive virtual Coins only. Coins and rewards are in-app entertainment credits and cannot be converted into cash.', style: TextStyle(color: Colors.white60, height: 1.35, fontSize: 12))),
                ]),
              ),
            ],
          );
  ...
  Widget _gameCoinCard(int coins) => Container(
    padding: const EdgeInsets.all(20),
    decoration: BoxDecoration(
      gradient: const LinearGradient(colors: [Color(0xFF3A1757), Color(0xFF171027)]),
      borderRadius: BorderRadius.circular(22),
      border: Border.all(color: const Color(0x44FFD76A)),
      boxShadow: const [BoxShadow(color: Color(0x44100018), blurRadius: 18, offset: Offset(0, 8))],
    ),
    child: Row(children: [
      Container(width: 58, height: 58, decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [Color(0xFFFFE08A), Color(0xFFFF9F43)])), child: const Center(child: Text('🪙', style: TextStyle(fontSize: 30)))),
      const SizedBox(width: 14),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('AVAILABLE COINS', style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 1)), const SizedBox(height: 4), Text('$coins', style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.w900)), const Text('Virtual entertainment balance', style: TextStyle(color: Colors.white54, fontSize: 11))])),
  ...
  Widget _gameTile(BuildContext context, String title, String subtitle, String emoji, Color accent, int entry, Widget page) => Container(
    margin: const EdgeInsets.only(bottom: 12),
    decoration: BoxDecoration(color: const Color(0xFF1B1425), borderRadius: BorderRadius.circular(20), border: Border.all(color: accent.withAlpha(55))),
    child: InkWell(
      borderRadius: BorderRadius.circular(20),
      onTap: () => openPage(context, page),
      child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
        Container(width: 62, height: 62, decoration: BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [accent, accent.withAlpha(120)])), child: Center(child: Text(emoji, style: const TextStyle(fontSize: 31)))),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(subtitle, style: const TextStyle(color: Colors.white54, fontSize: 11, height: 1.25)), const SizedBox(height: 7), Text('Entry $entry Coins', style: TextStyle(color: accent, fontSize: 11, fontWeight: FontWeight.w900))])),
...
int _gameInt(dynamic value) => value is num ? value.toInt() : int.tryParse('$value') ?? 0;

Future<bool> _spendGameCoins({required int stake, required String gameType}) async {
  final uid = FirebaseAuth.instance.currentUser?.uid;
  if (uid == null || stake <= 0) return false;
  final db = FirebaseFirestore.instance;
  final userRef = db.collection('users').doc(uid);
  final eventRef = db.collection('game_events').doc();
  try {
    await db.runTransaction((tx) async {
      final snap = await tx.get(userRef);
      final coins = _gameInt(snap.data()?['coins']);
      if (coins < stake) throw StateError('INSUFFICIENT_COINS');
      tx.update(userRef, {'coins': coins - stake, 'updatedAt': FieldValue.serverTimestamp()});
      tx.set(eventRef, {'uid': uid, 'gameType': gameType, 'stake': stake, 'reward': 0, 'result': 'started', 'createdAt': FieldValue.serverTimestamp()});
    });
    return true;
  } catch (_) {
    return false;
...
Future<void> _finishGame({required int stake, required int reward, required String gameType, required String result}) async {
  final uid = FirebaseAuth.instance.currentUser?.uid;
  if (uid == null) return;
  final db = FirebaseFirestore.instance;
  final userRef = db.collection('users').doc(uid);
  await db.runTransaction((tx) async {
    final snap = await tx.get(userRef);
    final coins = _gameInt(snap.data()?['coins']);
    tx.update(userRef, {'coins': coins + reward, 'gameSpent': FieldValue.increment(stake), 'gameWon': FieldValue.increment(reward), 'updatedAt': FieldValue.serverTimestamp()});
    tx.set(db.collection('game_events').doc(), {'uid': uid, 'gameType': gameType, 'stake': stake, 'reward': reward, 'result': result, 'createdAt': FieldValue.serverTimestamp()});
  });
}

class _GameBaseState<T extends StatefulWidget> extends State<T> {
  bool busy = false;
  String result = '';
  int lastReward = 0;
  Future<bool> start(int stake, String type) async {
    if (busy) return false;
    setState(() => busy = true);
    final ok = await _spendGameCoins(stake: stake, gameType: type);
    if (!ok && mounted) { setState(() => busy = false); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Not enough Coins'))); }
    return ok;
  }
  Future<void> finish(int stake, int reward, String type, String outcome) async {
    try { await _finishGame(stake: stake, reward: reward, gameType: type, result: outcome); } finally { if (mounted) setState(() { busy = false; lastReward = reward; result = outcome; }); }
  }
}
...
Widget _gameScaffold(BuildContext context, String title, String emoji, int stake, double angle, VoidCallback action, String result, bool busy) => Scaffold(
  ...
  body: ListView(padding: const EdgeInsets.all(20), children: [
    ...
    const SizedBox(height: 30),
    Center(child: Text('Entry $stake Coins', style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.w800))),
    const SizedBox(height: 16),
  ...
  return Scaffold(
    ...
    body: ListView(
      ...
      children: [
        ...
        const SizedBox(height: 20),
        const Center(child: Text('Entry 200 Coins', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.w700))),
        if (result.isNotEmpty)
          Padding(
...
class GameHistoryPage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    return Scaffold(appBar: AppBar(title: const Text('Game History')), body: uid == null ? const Center(child: Text('Please login again')) : StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection('game_events').where('uid', isEqualTo: uid).orderBy('createdAt', descending: true).limit(50).snapshots(), builder: (context, snap) { if (snap.hasError) return Center(child: Padding(padding: const EdgeInsets.all(20), child: Text('Game history unavailable. If Firestore asks for an index, create the suggested index.\n\n${snap.error}', textAlign: TextAlign.center))); if (!snap.hasData) return const Center(child: CircularProgressIndicator()); if (snap.data!.docs.isEmpty) return const Center(child: Text('No games played yet.')); return ListView.separated(padding: const EdgeInsets.all(16), itemCount: snap.data!.docs.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (_, i) { final d = snap.data!.docs[i].data(); final reward = _gameInt(d['reward']); final stake = _gameInt(d['stake']); return ListTile(tileColor: card, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)), leading: const CircleAvatar(child: Text('🎮')), title: Text('${d['gameType'] ?? 'Game'}'), subtitle: Text('Entry $stake • ${d['result'] ?? ''}'), trailing: Text('+$reward', style: TextStyle(color: reward > 0 ? Colors.green : Colors.white54, fontWeight: FontWeight.w900))); }); }); }
}
