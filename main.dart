...
// ZEGO AppID is public. Keep AppSign in GitHub Secrets and inject it at build time.
const int _zegoAppId = 1236782539;
const String _zegoAppSign = String.fromEnvironment('ZEGO_APP_SIGN');
      ...
      child: InkWell(
        onTap: () => openPage(
          ...
          LiveRoomPage(
            ...
            photoUrl: photo,
            liveId: (data['liveId'] ?? data['uid'] ?? 'wika_${name.hashCode}').toString(),
          ),
        ),
        borderRadius: BorderRadius.circular(22),
        child: Container(
          ...
          child: ListTile(
            ...
            onTap: () => openPage(
              ...
              LiveRoomPage(
                ...
                viewers: '$viewers viewers',
                photoUrl: photo,
                liveId: (x['liveId'] ?? x['uid'] ?? docs[index].id).toString(),
              ),
            ),
          ),
        );
...
class LiveRoomPage extends StatelessWidget {
  ...
  String get _userId {
    final user = FirebaseAuth.instance.currentUser;
    return user?.uid ?? 'guest_${DateTime.now().millisecondsSinceEpoch}';
  }
  ...
  String get _userName {
    ...
    return 'WikaLive User';
  }

  Future<void> _markHostEnded() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
    try {
      ...
      // Ending the ZEGO room must not be blocked by a Firestore write failure.
    }
  }

  @override
  Widget build(BuildContext context) {
    if (_zegoAppSign.trim().isEmpty) {
      return Scaffold(
        backgroundColor: const Color(0xFF070813),
        appBar: AppBar(
          backgroundColor: const Color(0xFF070813),
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: const Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.key_off_rounded, color: Color(0xFFFF5B8D), size: 48),
                  SizedBox(height: 14),
                  Text(
                    'ZEGO AppSign is not configured',
                    textAlign: TextAlign.center,
                    style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900),
                  ),
                  SizedBox(height: 8),
                  Text(
                    'Add ZEGO_APP_SIGN to GitHub Secrets. Do not put the AppSign in main.dart.',
                    textAlign: TextAlign.center,
                    style: TextStyle(color: Color(0xFF9A9DB2), fontSize: 12, height: 1.4),
                  ),
                ],
              ),
            ),
      ...
      body: ZegoUIKitPrebuiltLiveStreaming(
        ...
        events: ZegoUIKitPrebuiltLiveStreamingEvents(
          onEnded: (event, defaultAction) async {
            if (isHost) await _markHostEnded();
            defaultAction();
          },
          ...
          onError: (error) {
            debugPrint('ZEGO live error: $error');
          },
        ),
      ),
    );
...
class _HostPageState extends State<HostPage> {
  final title = TextEditingController(text: 'My WikaLive');
  bool starting = false;
  ...
  String _userName(User user) {
    ...
    final email = user.email?.trim();
    if (email != null && email.isNotEmpty) return email.split('@').first;
    return 'WikaLive Host';
  }

  Future<void> _startLive() async {
    if (_zegoAppSign.trim().isEmpty) {
      msg(context, 'ZEGO_APP_SIGN is not configured in GitHub Secrets.');
      return;
    }
    ...
    final liveId = 'live_${user.uid}_${DateTime.now().millisecondsSinceEpoch}';
    ...
    try {
      ...
      await userRef.set({
        'isLive': true,
        'liveId': liveId,
        'liveTitle': title.text.trim().isEmpty ? 'My WikaLive' : title.text.trim(),
      ...
      // Safety cleanup in case the SDK exits without firing onEnded.
      await userRef.set({
        'isLive': false,
        'liveId': FieldValue.delete(),
        'liveEndedAt': FieldValue.serverTimestamp(),
      ...
      if (mounted) msg(context, 'Could not start live: $e');
    } finally {
      ...
      body: ListView(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                colors: [Color(0xFF241047), Color(0xFF101329)],
                begin: Alignment.topLeft,
                end: Alignment.bottomRight,
              ),
              borderRadius: BorderRadius.circular(28),
              border: Border.all(color: const Color(0xFF6E36A8)),
            ),
            child: const Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Icon(Icons.videocam_rounded, color: Color(0xFFD88CFF), size: 46),
                SizedBox(height: 14),
                Text('Start a real live stream', style: TextStyle(color: Colors.white, fontSize: 23, fontWeight: FontWeight.w900)),
                SizedBox(height: 7),
                Text('Camera, microphone, audience chat and realtime video are powered by ZEGO.', style: TextStyle(color: Color(0xFFA7A9BC), height: 1.45)),
              ],
            ),
          ),
          const SizedBox(height: 18),
          TextField(
            ...
            decoration: InputDecoration(
              ...
              labelStyle: const TextStyle(color: Color(0xFF9A9DB2)),
              prefixIcon: const Icon(Icons.title_rounded, color: Color(0xFFD88CFF)),
              filled: true,
          ...
          SizedBox(
            ...
            child: FilledButton.icon(
              onPressed: starting ? null : _startLive,
              icon: starting
                  ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                  : const Icon(Icons.radio_rounded),
              label: Text(starting ? 'Starting...' : 'Start Live Now', style: const TextStyle(fontWeight: FontWeight.w900)),
            ),
          ),
        ],
      ),
