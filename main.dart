...
Future<void> main() async {
  ...
  await _ensureCurrentUserPublicId();
...
String wikaNumericUserId(String uid) {
  // Stable 8-digit public ID derived from the Firebase UID.
  // The Firebase UID remains the real backend identity and is never editable.
  var hash = 2166136261;
  for (final codeUnit in uid.codeUnits) {
    hash = ((hash ^ codeUnit) * 16777619) & 0xFFFFFFFF;
  }
  final number = 10000000 + (hash % 90000000);
  return number.toString();
}

Future<void> _ensureCurrentUserPublicId() async {
  ...
  try {
    ...
    if ((data['userId'] ?? '').toString().trim().isEmpty) {
      await ref.set({
        'uid': user.uid,
        'userId': wikaNumericUserId(user.uid),
    ...
    try {
      if (login) {
        ...
        await FirebaseFirestore.instance
          ...
          'userId': wikaNumericUserId(credential.user!.uid),
          'name': name.text.trim(),
          'email': normalizedEmail,
          'coins': 0,
          'diamonds': 0,
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        final shortId = (d['userId'] ?? wikaNumericUserId(uid)).toString();
...
class _StartLivePageState extends State<StartLivePage> {
  ...
  String selectedTab = 'Topic';
  XFile? cover;

  final titleController = TextEditingController(text: 'Welcome to my Live Stream! ✨');
  ...
  // No game category is exposed in WikaLive.
  static const categories = <String>[
    'Chat', 'Music', 'Talent', 'Daily', 'PK', 'Party', 'Other',
  ];
        ...
        builder: (context, setSheetState) => SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Center(child: Container(width: 42, height: 4, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(9)))),
                const SizedBox(height: 16),
                const Text('Before you go live', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)),
                const SizedBox(height: 6),
                const Text('Keep WikaLive safe and welcoming for everyone.', style: TextStyle(color: Color(0xFF9D9BA8))),
                const SizedBox(height: 12),
                Flexible(
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) => Padding(
                      ...
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Container(width: 25, height: 25, alignment: Alignment.center, decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [Color(0xFFFF39C8), Color(0xFF7B35FF)])), child: Text('${i + 1}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 11))),
                        const SizedBox(width: 10),
                        Expanded(child: Text(rules[i], style: const TextStyle(color: Color(0xFFE8E6EF), fontSize: 13, height: 1.35))),
                      ]),
                    ),
                  ),
                ),
                CheckboxListTile(
                  ...
                  value: accepted,
                  onChanged: (v) => setSheetState(() => accepted = v ?? false),
                  activeColor: const Color(0xFFD64CFF),
                  title: const Text('I agree to the live rules.', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                ),
                SizedBox(
                  width: double.infinity,
                  height: 52,
                  child: DecoratedBox(
                    decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFFF39C8), Color(0xFF7B35FF)]), borderRadius: BorderRadius.circular(16)),
                    child: ElevatedButton(
                      onPressed: accepted ? () => Navigator.pop(sheetContext, true) : null,
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, disabledBackgroundColor: Colors.transparent, shadowColor: Colors.transparent),
                      child: const Text('CONTINUE', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, letterSpacing: .8)),
                    ),
                  ),
                ),
              ],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Live Host Center', style: TextStyle(fontWeight: FontWeight.w900)),
        actions: [IconButton(icon: const Icon(Icons.settings_outlined), onPressed: () => msg(context, 'Live settings'))],
      ),
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        stream: uid.isNotEmpty ? FirebaseFirestore.instance.collection('users').doc(uid).snapshots() : null,
        builder: (context, snapshot) {
          final data = snapshot.data?.data() ?? <String, dynamic>{};
          final name = (data['name'] ?? user?.displayName ?? 'Host Name').toString();
          final publicUserId = (data['userId'] ?? wikaNumericUserId(uid)).toString();
            ...
            children: [
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    _hostProfileCard(name, publicUserId, photoUrl, level),
                    const SizedBox(height: 14),
                    _tabBar(),
                    const SizedBox(height: 16),
                    if (selectedTab == 'Topic') ...[
                      _titleCard(),
                      const SizedBox(height: 14),
                      _sectionTitle('Choose your live topic'),
                      const SizedBox(height: 10),
                      _categoryChips(),
                    ] else if (selectedTab == 'Live Data') ...[
                      _infoGrid([
                        _infoTile(Icons.people_alt_outlined, 'Guests', allowGuest ? 'Allowed' : 'Off'),
                        _infoTile(Icons.chat_bubble_outline_rounded, 'Comments', allowComments ? 'Allowed' : 'Off'),
                        _infoTile(Icons.location_on_outlined, 'Location', showLocation ? 'Visible' : 'Hidden'),
                      ]),
                    ] else if (selectedTab == 'Room Settings') ...[
                    ...
                    ] else if (selectedTab == 'Beauty') ...[
                      _beautyPanel(),
                    ] else if (selectedTab == 'Ranking') ...[
                      _infoGrid([_infoTile(Icons.emoji_events_outlined, 'Ranking', 'Ready'), _infoTile(Icons.diamond_outlined, 'Gifts', 'Live'), _infoTile(Icons.trending_up_rounded, 'Popular', 'Live')]),
                    ] else ...[
                      _infoGrid([_infoTile(Icons.bolt_rounded, 'PK Mode', 'Available'), _infoTile(Icons.people_alt_outlined, 'Guests', '3 max'), _infoTile(Icons.mic_rounded, 'Audio', 'On')]),
                    ],
                    const SizedBox(height: 18),
  ...
  Widget _hostProfileCard(String name, String id, String photoUrl, int level) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [Color(0xFF24104F), Color(0xFF53178A)], begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: const Color(0x554D2B80)),
        boxShadow: const [BoxShadow(color: Color(0x443B1A70), blurRadius: 18, offset: Offset(0, 6))],
      ),
      child: Row(children: [
        Stack(children: [
          Container(padding: const EdgeInsets.all(2), decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [Color(0xFFFF39C8), Color(0xFF7B35FF)])), child: CircleAvatar(radius: 36, backgroundColor: const Color(0xFF17132A), backgroundImage: photoUrl.isNotEmpty ? NetworkImage(photoUrl) : null, child: photoUrl.isEmpty ? const Icon(Icons.person, color: Colors.white70, size: 40) : null)),
          Positioned(right: 0, bottom: 0, child: Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(color: const Color(0xFFFF2A7A), borderRadius: BorderRadius.circular(10), border: Border.all(color: const Color(0xFF24104F), width: 2)), child: const Text('LIVE', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900)))),
        ]),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Text(name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w900))), const Icon(Icons.verified_rounded, color: Color(0xFF45B8FF), size: 19)]),
          const SizedBox(height: 4),
          Text('ID: $id', style: const TextStyle(color: Color(0xFFC0B3D8), fontSize: 13, fontWeight: FontWeight.w700)),
          const SizedBox(height: 9),
          Wrap(spacing: 7, runSpacing: 6, children: [
            _badge('Lv.$level', const Color(0xFFFFB800), Colors.black),
            _badge('VIP Host', const Color(0xFF9C27B0), Colors.white),
            _badge('Online', const Color(0xFF1FD89A), Colors.black),
          ]),
        ])),
      ]),
    );
  ...
  Widget _badge(String text, Color bg, Color fg) => Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 4), decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(11)), child: Text(text, style: TextStyle(color: fg, fontSize: 10, fontWeight: FontWeight.w900)));

  Widget _tabBar() {
    const tabs = ['Topic', 'Live Data', 'Room Settings', 'Beauty', 'Ranking', 'PK Mode'];
    return SizedBox(height: 42, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: tabs.length, separatorBuilder: (_, __) => const SizedBox(width: 8), itemBuilder: (_, i) {
      final active = selectedTab == tabs[i];
      return InkWell(onTap: () => setState(() => selectedTab = tabs[i]), borderRadius: BorderRadius.circular(14), child: Container(padding: const EdgeInsets.symmetric(horizontal: 14), alignment: Alignment.center, decoration: BoxDecoration(gradient: active ? const LinearGradient(colors: [Color(0xFF8B3FD9), Color(0xFFE94D9A)]) : null, color: active ? null : const Color(0xFF111426), borderRadius: BorderRadius.circular(14), border: Border.all(color: active ? Colors.transparent : const Color(0xFF282C45))), child: Text(tabs[i], style: TextStyle(color: active ? Colors.white : const Color(0xFFA6A5B7), fontWeight: FontWeight.w800, fontSize: 12))));
    }));
  }

  Widget _titleCard() => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF111426), borderRadius: BorderRadius.circular(17), border: Border.all(color: const Color(0xFF292D4A))), child: TextField(controller: titleController, maxLength: 40, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700), decoration: const InputDecoration(border: InputBorder.none, counterStyle: TextStyle(color: Color(0xFF73788E)), labelText: 'Live Room Title', labelStyle: TextStyle(color: Color(0xFF8F93A8)), prefixIcon: Icon(Icons.edit_note_rounded, color: Color(0xFFB65BEA)))));

  Widget _sectionTitle(String text) => Text(text, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900));

  Widget _categoryChips() => Wrap(spacing: 9, runSpacing: 9, children: categories.map((x) { final active = category == x; return ChoiceChip(label: Text(x), selected: active, onSelected: (_) => setState(() => category = x), selectedColor: const Color(0xFF9A3DFF), backgroundColor: const Color(0xFF111426), labelStyle: TextStyle(color: active ? Colors.white : const Color(0xFFAAA9B8), fontWeight: FontWeight.w800), side: BorderSide(color: active ? const Color(0xFFE66BFF) : const Color(0xFF2C3048))); }).toList());

  Widget _controlsGrid() => GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 3, crossAxisSpacing: 10, mainAxisSpacing: 10, childAspectRatio: 1.05, children: [
    _controlTile(Icons.edit_note_rounded, 'Topic', true, () => setState(() => selectedTab = 'Topic')),
    _controlTile(Icons.people_alt_outlined, 'Guests', allowGuest, () => setState(() => allowGuest = !allowGuest)),
    _controlTile(Icons.mic_rounded, 'Microphone', true, () => msg(context, 'Microphone will be controlled in the live room.')),
    _controlTile(Icons.face_retouching_natural, 'Beauty', beautyOn, () => setState(() { beautyOn = !beautyOn; selectedTab = 'Beauty'; })),
    _controlTile(Icons.campaign_outlined, 'Announcement', false, () => msg(context, 'Room announcement will be available after going live.')),
    _controlTile(Icons.bolt_rounded, 'PK Match', false, () => msg(context, 'PK Match will be available in the live room.')),
  ]);

  Widget _controlTile(IconData icon, String label, bool active, VoidCallback onTap) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(17), child: Container(decoration: BoxDecoration(color: const Color(0xFF111426), borderRadius: BorderRadius.circular(17), border: Border.all(color: active ? const Color(0xFF8B3FD9) : const Color(0xFF292D4A))), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: active ? const Color(0xFFE94D9A) : const Color(0xFF8F93A8), size: 27), const SizedBox(height: 7), Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: active ? Colors.white : const Color(0xFF9EA1B1), fontSize: 11, fontWeight: FontWeight.w800))])));

  Widget _beautyPanel() => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF111426), borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xFF292D4A)),), child: Row(children: [Container(width: 52, height: 52, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFF25153D)), child: const Icon(Icons.auto_awesome_rounded, color: Color(0xFFE15CFF))), const SizedBox(width: 12), const Expanded(child: Text('Beauty preview', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),), Switch(value: beautyOn, activeColor: const Color(0xFFD64CFF), onChanged: (v) => setState(() => beautyOn = v))]));

  Widget _infoGrid(List<Widget> children) => Row(children: children.map((x) => Expanded(child: Padding(padding: const EdgeInsets.only(right: 8), child: x))).toList());

  Widget _infoTile(IconData icon, String title, String value) => Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFF111426), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFF292D4A))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, color: const Color(0xFFD15CFF)), const SizedBox(height: 8), Text(title, style: const TextStyle(color: Color(0xFF989AAA), fontSize: 11)), const SizedBox(height: 2), Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12))]));

  Widget _switchTile(String text, bool value, ValueChanged<bool> onChanged, IconData icon) => Container(margin: const EdgeInsets.only(bottom: 9), decoration: BoxDecoration(color: const Color(0xFF111426), borderRadius: BorderRadius.circular(17), border: Border.all(color: const Color(0xFF292D4A))), child: SwitchListTile(secondary: Icon(icon, color: const Color(0xFFC96BFF)), title: Text(text, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700)), value: value, onChanged: onChanged, activeColor: const Color(0xFFD64CFF)));
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: const Color(0xFF111426),
          borderRadius: BorderRadius.circular(18),
          border: Border.all(color: const Color(0xFF292D4A)),
        ),
        child: Stack(
          fit: StackFit.expand,
          children: [
            if (cover != null)
              ...
              const Center(
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(Icons.add_a_photo_rounded, color: Color(0xFFD15CFF), size: 30),
                    SizedBox(height: 6),
                    Text('Add live cover', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
                  ],
                ),
              ),
            if (cover != null)
  ...
  Widget _bottomStartBar() {
    return Container(
      padding: const EdgeInsets.fromLTRB(16, 10, 16, 16),
      decoration: const BoxDecoration(
        color: Color(0xFF0F1120),
        border: Border(top: BorderSide(color: Color(0xFF22263D))),
      ),
      child: SizedBox(
        width: double.infinity,
        height: 54,
        child: DecoratedBox(
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(17),
            boxShadow: const [BoxShadow(color: Color(0x553F1A70), blurRadius: 16, offset: Offset(0, 5))],
          ),
          child: ElevatedButton(
            onPressed: starting ? null : _launchLive,
            style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, shadowColor: Colors.transparent),
            child: Text(
              starting ? 'STARTING LIVE…' : 'GO LIVE NOW',
...
class _RealHostLivePageState extends State<RealHostLivePage> {
  ...
  void initState() {
    ...
    // Lock the host session to portrait. Without this, the device's
    // rotation sensor can still report a landscape/upside-down reading
    // while the Flutter UI stays portrait, and the ZEGO camera capture
    // follows that sensor reading — producing the sideways/upside-down
    // preview seen on some devices. Locking orientation here keeps the
    // camera feed upright and matching the on-screen portrait layout.
...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
  ...
  static const List<Map<String, dynamic>> _demoMessages = [
    {'name': 'Rohit', 'msg': 'Hello bro ❤️', 'level': 28},
    {'name': 'Sameer', 'msg': 'Amazing live! ✨', 'level': 36},
    {'name': 'Neha', 'msg': 'Sent you a heart ❤️', 'level': 45},
    {'name': 'Arman', 'msg': 'Nice bro! 👍', 'level': 22},
  ];
        ...
        return Stack(
          ...
          children: [
            Positioned(
              top: top + 14,
              left: 10,
              child: SizedBox(width: media.size.width * .33, child: _hostCard()),
            ),
            Positioned(
              top: top + 2,
              left: media.size.width * .355,
              child: SizedBox(width: 112, height: 112, child: _guardianBadge()),
            ),
            Positioned(
              top: top + 8,
            ...
            Positioned(
              ...
              child: _glass(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(width: 5),
                  Text(_shortNumber(audienceCount),
            ...
            Positioned(top: top + 118, left: 14, child: _topContributionCard()),
            Positioned(top: top + 118, right: 12, child: _liveDiamondCard()),
            Positioned(
              right: 12,
                  ...
                  if (visible.isEmpty) {
                    return Column(
                      ...
                      children: _demoMessages
                          .map((m) => _chatBubble(m['name'] as String, m['msg'] as String, '', m['level'] as int))
                          .toList(),
                    );
                  }
                  return Column(
                    ...
                    children: List.generate(hearts, (index) {
                      ...
                      return Positioned(
                        ...
                        child: Icon(
                          ...
                          shadows: const [Shadow(color: Color(0xFFB000FF), blurRadius: 14)],
                        ),
                      );
                    }),
                  ),
                ...
                child: DecoratedBox(
                  decoration: const BoxDecoration(
                    gradient: LinearGradient(
                      ...
                      colors: [Colors.transparent, Color(0x66000000), Color(0xF5000000)],
                      stops: [0.0, 0.28, 1.0],
                    ),
                  ),
                ),
      ...
      builder: (context, snapshot) {
        ...
        return Row(
          ...
          children: List.generate(docs.length, (index) {
            ...
            final level = d['level'] is num ? (d['level'] as num).toInt() : 1;
            final diamondsSent = d['diamondsSent'] is num
                ? (d['diamondsSent'] as num).toInt()
                : 0;
            return _rankDataAvatar(index + 1, name, photo, level, diamondsSent);
          }),
        );
      },
    );
  ...
  Widget _rankDataAvatar(int rank, String name, String photo, int level, int diamondsSent) {
    final ring = rank == 1
          ...
          Stack(
            ...
            children: [
              ...
              Container(
                ...
                child: _avatar(url: photo, name: name, radius: 24),
              ),
            ],
          ),
          const SizedBox(height: 3),
            ...
            Row(
              ...
              children: [
                const Icon(Icons.card_giftcard_rounded,
                    color: Color(0xFFFF4DB8), size: 11),
                const SizedBox(width: 2),
                Text(_shortNumber(diamondsSent),
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 10,
                      fontWeight: FontWeight.w900,
            ...
            children: [
              Positioned(
                ...
                child: Transform.rotate(
                  ...
                  child: const Icon(
                    ...
                    size: 42,
                    shadows: [
                      Shadow(color: Color(0xFFB63CFF), blurRadius: 8),
                    ],
                  ),
                ),
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: const [
                    BoxShadow(color: Color(0xB28E3CFF), blurRadius: 14, spreadRadius: 2),
                    BoxShadow(color: Color(0x447F35FF), blurRadius: 24),
                  ],
                ),
                child: const Stack(
                  alignment: Alignment.center,
  ...
  Widget _bottomControls() {
    return SizedBox(
      ...
      child: Row(
        ...
        children: [
          _bottomButton(Icons.more_horiz_rounded, 'More',
              () => _openMoreTools(context)),
          _bottomButton(Icons.auto_awesome_rounded, 'Beauty', _toggleBeauty,
              active: beautyOn),
          _bottomButton(
              micOn ? Icons.mic_rounded : Icons.mic_off_rounded, 'Mic',
              _toggleMic,
              active: micOn),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(icon, color: active ? Colors.white : const Color(0xFFFF55B8), size: 26),
            ),
            const SizedBox(height: 4),
            Text(
              label,
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: const [
                  BoxShadow(color: Color(0x88523FF), blurRadius: 12, spreadRadius: 1),
                ],
              ),
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          final level = d['level'] is num ? (d['level'] as num).toInt() : 1;
          final id = (d['userId'] ?? wikaNumericUserId(u.uid)).toString();
          final raw = d['photos'];
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _load() async {
    ...
    if ((d['userId'] ?? '').toString().trim().isEmpty) {
      await FirebaseFirestore.instance.collection('users').doc(u.uid).set({
        'uid': u.uid,
        'userId': wikaNumericUserId(u.uid),
      }, SetOptions(merge: true));
    }
  ...
  Widget _userIdDisplay() {
    final u = FirebaseAuth.instance.currentUser;
    final id = u == null ? '' : wikaNumericUserId(u.uid);
    return Padding(
...
class _UserSearchPageState extends State<UserSearchPage> {
  ...
  Future<void> _search() async {
    ...
    try {
      // Search the immutable numeric public ID.  Support both String and
      // numeric Firestore values, plus older documents that do not yet have
      // userId stored by deriving the same ID from the Firebase UID.
      final users = FirebaseFirestore.instance.collection('users');
      final seen = <String>{};
      final matches = <DocumentSnapshot<Map<String, dynamic>>>[];

      void addMatch(DocumentSnapshot<Map<String, dynamic>> doc) {
        if (seen.add(doc.id)) matches.add(doc);
      }

      // Always check the signed-in account directly first. This guarantees
      // that the exact numeric ID shown in My Space can find that account,
      // even if the Firestore collection query is missing/old for that user.
      final current = FirebaseAuth.instance.currentUser;
      if (current != null) {
        final ownRef = users.doc(current.uid);
        final ownSnap = await ownRef.get();
        final ownData = ownSnap.data();
        if (ownSnap.exists && ownData != null) {
          final ownStored = (ownData['userId'] ?? '').toString().trim();
          final ownPublic = (ownData['publicId'] ?? ownData['numericUserId'] ?? ownData['shortId'] ?? '').toString().trim();
          final ownDerived = wikaNumericUserId(current.uid);
          if (ownStored == query || ownPublic == query || ownDerived == query) {
            addMatch(ownSnap);
          }
        }
      }

      final docsByString = await users.where('userId', isEqualTo: query).limit(20).get();
      final docsByNumber = int.tryParse(query) == null
          ? null
          : await users.where('userId', isEqualTo: int.parse(query)).limit(20).get();

      for (final doc in docsByString.docs) {
        addMatch(doc);
      }
      if (docsByNumber != null) {
        for (final doc in docsByNumber.docs) {
          addMatch(doc);
        }
      }

      // Legacy/fallback support: if userId was never stored, compare the
      // same deterministic ID generated from the Firebase document UID.
      if (matches.isEmpty) {
        final snap = await users.get();
        for (final doc in snap.docs) {
          final data = doc.data();
          final stored = (data['userId'] ?? '').toString().trim();
          final publicId = (data['publicId'] ?? data['numericUserId'] ?? data['shortId'] ?? '').toString().trim();
          final derived = wikaNumericUserId(doc.id);
          final storedMatch = stored == query ||
              (query.length >= 4 && stored.startsWith(query));
          final publicIdMatch = publicId == query ||
              (query.length >= 4 && publicId.startsWith(query));
          final derivedMatch = derived == query ||
              (query.length >= 4 && derived.startsWith(query));
          if (storedMatch || publicIdMatch || derivedMatch) {
            addMatch(doc);
          }
        }
      }
      ...
      setState(() {
        searching = false;
        searchError = 'Unable to search User ID';
      });
    }
  }
    ...
    body: ListView(
      ...
      children: [
        TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Search by numeric User ID',
            labelText: 'User ID',
        ...
        if (searching)
        ...
        else if (c.text.trim().isNotEmpty && results.isEmpty)
          const Padding(
            padding: EdgeInsets.all(20),
            child: Text('No user found for this ID'),
          )
        else if (results.isNotEmpty)
          Column(
            children: results.map((d) {
              final data = d.data() ?? <String, dynamic>{};
              // Show the exact immutable ID stored for the account; fall
              // back to the deterministic ID only for legacy documents.
              final id = (data['userId'] ?? data['publicId'] ?? data['numericUserId'] ?? wikaNumericUserId(d.id)).toString();
              return ListTile(
                leading: CircleAvatar(
                ...
                title: Text((data['name'] ?? 'User').toString()),
                subtitle: Text('ID: $id'),
                onTap: () => openPage(context, UserProfilePage(uid: d.id)),
              );
            }).toList(),
          )
        else
          const Padding(
            padding: EdgeInsets.all(20),
            child: Text('Enter the numeric User ID to search'),
          ),
      ],
    ),
  );
