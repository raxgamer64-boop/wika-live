    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Google login failed: ${e.message ?? e.code}')),
        );
      }
    } catch (e) {
      if (mounted) {
...
class _LiveScreenState extends State<LiveScreen> {
  ...
  Query<Map<String, dynamic>> _base() =>
      FirebaseFirestore.instance.collection('users').where('isLive', isEqualTo: true);
  ...
  Stream<List<QueryDocumentSnapshot<Map<String, dynamic>>>> _hosts() {
    return _base().snapshots().map((s) {
      ...
      DateTime d(dynamic v) => v is Timestamp ? v.toDate() : DateTime.fromMillisecondsSinceEpoch(0);

      if (section == 'Popular' || section == 'Explore') {
        list.sort((a, b) => n(b.data()['giftReceived']).compareTo(n(a.data()['giftReceived'])));
      }
      if (section == 'Beauty') {
        list.sort((a, b) => n(b.data()['beautyScore'] ?? b.data()['qualityScore'])
            .compareTo(n(a.data()['beautyScore'] ?? a.data()['qualityScore'])));
      }
      if (section == 'New Host') {
        final cutoff = DateTime.now().subtract(const Duration(days: 7));
        list.removeWhere((x) {
          final v = x.data()['createdAt'];
          return v is! Timestamp || v.toDate().isBefore(cutoff);
        });
        list.sort((a, b) => d(b.data()['createdAt']).compareTo(d(a.data()['createdAt'])));
      }
      return list;
    });
  }
    ...
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [Color(0xFF0B0811), Color(0xFF140C1D)],
        ),
      ),
      child: SafeArea(
        child: Column(
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  const Text('Live', style: TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900)),
                  const Spacer(),
                  _circleAction(Icons.search_rounded, () => openPage(context, const UserSearchPage())),
                  const SizedBox(width: 10),
                  _circleAction(Icons.workspace_premium_rounded, () => openPage(context, const RankingPage())),
                ],
              ),
            ),
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) => GestureDetector(
                  onTap: () {
                    ...
                    if (sections[i] == 'Country') openPage(context, const CountryLivePage());
                  },
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      gradient: section == sections[i]
                          ? const LinearGradient(colors: [Color(0xFFB85CFF), Color(0xFFFF4E9B)])
                          : null,
                      color: section == sections[i] ? null : const Color(0xFF21182B),
                      borderRadius: BorderRadius.circular(22),
                      border: Border.all(color: section == sections[i] ? Colors.transparent : const Color(0xFF392A45)),
                    ),
                    child: Text(sections[i], style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: section == sections[i] ? FontWeight.w900 : FontWeight.w700)),
                  ),
                ),
              ),
            ),
                ...
                builder: (context, snap) {
                  ...
                  if (section == 'Follow') {
                    return FutureBuilder<Set<String>>(
                      ...
                      builder: (c, f) {
                        final ids = f.data ?? {};
                        docs = docs.where((x) => ids.contains(x.id)).toList();
                        return _liveGrid(c, docs, 'No followed host is live right now.');
                      },
                    );
                  }
                  if (section == 'Nearby') return _nearby(context, docs);
                  return _liveGrid(context, docs, 'No live hosts found.');
                },
              ),
            ),
          ],
  ...
  Widget _circleAction(IconData icon, VoidCallback onTap) => InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: const Color(0xFF4A3556)),
          ),
          child: Icon(icon, color: Colors.white, size: 21),
        ),
      );

  Widget _liveGrid(BuildContext context, List<QueryDocumentSnapshot<Map<String, dynamic>>> docs, String empty) {
    if (docs.isEmpty) {
      return Center(child: Text(empty, style: const TextStyle(color: Colors.white60, fontWeight: FontWeight.w700)));
    }
    return GridView.builder(
      padding: const EdgeInsets.fromLTRB(14, 8, 14, 20),
      itemCount: docs.length,
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: .70),
      itemBuilder: (c, i) {
        final x = docs[i].data();
        final photo = (x['photos'] is List && (x['photos'] as List).isNotEmpty)
            ? (x['photos'] as List).first.toString()
            : (x['photoUrl'] ?? '').toString();
        return InkWell(
          onTap: () => openPage(c, LiveRoomPage(host: (x['name'] ?? 'Host').toString(), viewers: '${x['viewers'] ?? 0} viewers')),
          ...
          child: ClipRRect(
            ...
            child: Stack(
              ...
              children: [
                photo.isEmpty
                    ? Container(color: const Color(0xFF2A2032), child: const Icon(Icons.person_rounded, size: 72, color: Colors.white30))
                    : Image.network(photo, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(color: const Color(0xFF2A2032), child: const Icon(Icons.person_rounded, size: 72, color: Colors.white30))),
                ...
                Positioned(top: 9, left: 9, child: Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: const Color(0xFFE83F83), borderRadius: BorderRadius.circular(10)), child: const Text('LIVE', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w900)))),
                Positioned(top: 9, right: 9, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: Colors.black45, borderRadius: BorderRadius.circular(10)), child: Text('${x['viewers'] ?? 0}', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800)))),
                Positioned(left: 11, right: 11, bottom: 11, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text((x['name'] ?? 'Host').toString(), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 4),
                  Text('${x['countryFlag'] ?? '🌍'}  Live now', style: const TextStyle(color: Colors.white70, fontSize: 11, fontWeight: FontWeight.w700)),
                ])),
              ],
            ),
          ),
  ...
  Widget _nearby(BuildContext c, List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
    ...
    if (me == null) return _liveGrid(c, [], 'Please login again.');
    return FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (c, s) {
        ...
        if (lat == 0 && lon == 0) return const Center(child: Padding(padding: EdgeInsets.all(30), child: Text('Nearby live needs your location saved in your profile.', style: TextStyle(color: Colors.white60))));
        ...
        return _liveGrid(c, near, 'No live host found within 50 km.');
      },
    );
  }
                ...
                children: types.map((x) => Padding(
                  ...
                  child: GestureDetector(
                    ...
                    child: Column(
                      children: [
                        Text(
                          x == 'Live' ? '💗 Live' : x == 'Sender' ? '💰 Sender' : x == 'Party' ? '🏠 Party',
