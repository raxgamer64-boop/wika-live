...
class CountryLivePage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('Explore by Country')),body:GridView.builder(padding:const EdgeInsets.all(16),itemCount:countries.length,gridDelegate:const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:2,crossAxisSpacing:12,mainAxisSpacing:12,childAspectRatio:1.45),itemBuilder:(c,i){final p=countries[i].split(' ');return InkWell(onTap:()=>openPage(c,CountryHostsPage(country:countries[i].substring(5))),borderRadius:BorderRadius.circular(20),child:Container(padding:const EdgeInsets.all(16),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(20),boxShadow:const[BoxShadow(color:Color(0x10000000),blurRadius:14,offset:Offset(0,5))]),child:Row(children:[Text(p.first,style:const TextStyle(fontSize:34)),const SizedBox(width:10),Expanded(child:Text(countries[i].substring(5),style:const TextStyle(fontSize:15,fontWeight:FontWeight.w900)))])));}));
}
...
class _HostList extends StatelessWidget { final List<QueryDocumentSnapshot<Map<String,dynamic>>> docs; const _HostList({required this.docs}); @override Widget build(BuildContext c)=>docs.isEmpty?const Center(child:Text('No live hosts')):ListView.separated(padding:const EdgeInsets.all(16),itemCount:docs.length,separatorBuilder:(_,__)=>const SizedBox(height:10),itemBuilder:(c,i){final x=docs[i].data();final photo=(x['photoUrl']??'').toString();return ListTile(contentPadding:const EdgeInsets.all(10),tileColor:Colors.white,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(18)),leading:CircleAvatar(radius:28,backgroundImage:photo.isNotEmpty?NetworkImage(photo):null,child:photo.isEmpty?const Icon(Icons.person):null),title:Text((x['name']??'Host').toString(),style:const TextStyle(fontWeight:FontWeight.w900)),subtitle:Text('LIVE • ${x['viewers']??0} viewers'),trailing:const Icon(Icons.chevron_right_rounded),onTap:()=>openPage(c,LiveRoomPage(host:(x['name']??'Host').toString(),viewers:'${x['viewers']??0} viewers')));}); }

class PartyScreen extends StatelessWidget {
  const PartyScreen({super.key});
  static const rooms=[
    ['hotty hot 🔥','Make Friends','31','https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=600'],
    ['SINGH Agency','Music Party','4','https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=600'],
    ["Suman's room",'Gossip','3','https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=600'],
    ["Suhani's room",'Emotional Share','2','https://images.unsplash.com/photo-1511632765486-a01980e01a18?w=600'],
  ];
  @override Widget build(BuildContext context)=>SafeArea(child:ListView(padding:const EdgeInsets.fromLTRB(16,10,16,24),children:[
    Row(children:[const Expanded(child:Text('Party',style:TextStyle(fontSize:29,fontWeight:FontWeight.w900))),_top(Icons.search_rounded,()=>openPage(context,const FeaturePage(title:'Search Party'))),const SizedBox(width:8),_top(Icons.add_rounded,()=>openPage(context,const CreatePartyPage()))]),
    const SizedBox(height:14),
    Container(height:150,padding:const EdgeInsets.all(18),decoration:BoxDecoration(gradient:const LinearGradient(colors:[Color(0xFF25123D),Color(0xFF8A3E8F)]),borderRadius:BorderRadius.circular(24)),child:Row(children:[const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisAlignment:MainAxisAlignment.center,children:[Text('PARTY WORLD',style:TextStyle(color:Color(0xFFFFD76A),fontSize:11,fontWeight:FontWeight.w900,letterSpacing:1.5)),SizedBox(height:6),Text('Talk • Sing • Play • Make friends',style:TextStyle(color:Colors.white,fontSize:20,fontWeight:FontWeight.w900)),SizedBox(height:8),Text('Join a room and meet your next friend.',style:TextStyle(color:Colors.white70,fontSize:12))]),Text('🎤',style:TextStyle(fontSize:65))]),
    const SizedBox(height:16),
    SizedBox(height:42,child:ListView(scrollDirection:Axis.horizontal,children:['Popular','PK Battle','Music','Chat','Event'].map((x)=>Padding(padding:const EdgeInsets.only(right:8),child:ChoiceChip(label:Text(x),selected:x=='Popular',onSelected:(_){}})).toList())),
    const SizedBox(height:14),
    ...rooms.map((r)=>_room(context,r)),
  ]));
  Widget _top(IconData i,VoidCallback t)=>InkWell(onTap:t,borderRadius:BorderRadius.circular(22),child:Container(width:44,height:44,decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(22)),child:Icon(i)));
  Widget _room(BuildContext context,List<String> r)=>InkWell(onTap:()=>openPage(context,PartyRoomPage(title:r[0])),borderRadius:BorderRadius.circular(20),child:Container(margin:const EdgeInsets.only(bottom:12),padding:const EdgeInsets.all(11),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(20),boxShadow:const[BoxShadow(color:Color(0x10000000),blurRadius:12,offset:Offset(0,5))]),child:Row(children:[ClipRRect(borderRadius:BorderRadius.circular(16),child:Image.network(r[3],width:100,height:100,fit:BoxFit.cover,errorBuilder:(_,__,___)=>Container(width:100,height:100,color:const Color(0xFFE9DDF4),child:const Icon(Icons.groups_rounded,size:42)))),const SizedBox(width:13),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('🇮🇳  ${r[0]}',maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:17,fontWeight:FontWeight.w900)),const SizedBox(height:7),Text(r[1],style:const TextStyle(color:Colors.black45,fontWeight:FontWeight.w700)),const SizedBox(height:14),Row(children:[Container(padding:const EdgeInsets.symmetric(horizontal:10,vertical:6),decoration:BoxDecoration(color:const Color(0xFFFFE8F5),borderRadius:BorderRadius.circular(12)),child:Text(r[1],style:const TextStyle(color:Color(0xFFD84F9F),fontSize:11,fontWeight:FontWeight.w800))),const Spacer(),Text('👥 ${r[2]}',style:const TextStyle(fontWeight:FontWeight.w900))]))]))]));
}
...
class MeScreen extends StatelessWidget {
  ...
  Widget _creatorCard(BuildContext c)=>InkWell(onTap:()=>openPage(c,const HostPage()),borderRadius:BorderRadius.circular(21),child:Container(padding:const EdgeInsets.all(17),decoration:BoxDecoration(gradient:const LinearGradient(colors:[Color(0xFFF1E4FF),Colors.white]),borderRadius:BorderRadius.circular(21)),child:Row(children:[Container(width:50,height:50,decoration:BoxDecoration(color:const Color(0xFFE1C7FF),borderRadius:BorderRadius.circular(16)),child:const Icon(Icons.videocam_rounded,color:Color(0xFF7A42A9),size:27)),const SizedBox(width:12),const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Become a Creator',style:TextStyle(fontSize:16,fontWeight:FontWeight.w900)),SizedBox(height:4),Text('Go live, build followers and earn Diamonds.',style:TextStyle(color:Colors.black45,fontSize:11,fontWeight:FontWeight.w600))])),const Icon(Icons.chevron_right_rounded)]));
...
Future<bool> _spendGameCoins({
  ...
  // WikaLive games are free to start. Coins are never deducted as an
  // entry fee. The `stake` parameter is kept for compatibility with the
  // existing game screens, but is intentionally ignored here.
...
abstract class _GameBaseState<T extends StatefulWidget> extends State<T> {
  ...
  Future<bool> start(int stake, String gameType) async {
    ...
    // WikaLive games are free-to-start. No Coins are deducted here.
    return true;
  }
