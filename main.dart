...
class _GuestCallOverlayState extends State<_GuestCallOverlay> {
  ...
  void _toggleCamera() {
    ...
    if (mounted) setState(() => cameraOn = !cameraOn);
  }
  ...
  void _sendHeart() {
    if (!mounted) return;
    setState(() => hearts = (hearts + 1).clamp(0, 6));
    Future.delayed(const Duration(milliseconds: 1300), () {
      if (mounted && hearts > 0) setState(() => hearts--);
    });
  }

  Stream<DocumentSnapshot<Map<String, dynamic>>> _hostStream(String uid) =>
  ...
  String _photo(Map<String, dynamic> data, String? fallback) {
    final p = (data['photoUrl'] ?? fallback ?? '').toString().trim();
    return p;
  }
  ...
  Widget _avatar({String? url, String? name, double radius = 22}) {
    ...
    return CircleAvatar(
      radius: radius,
      backgroundColor: const Color(0xFF29243A),
      child: Text(
        ((name ?? '?').trim().isEmpty ? '?' : (name ?? '?').trim()[0]).toUpperCase(),
        style: TextStyle(color: Colors.white, fontSize: radius * .72, fontWeight: FontWeight.w900),
      ),
    );
  }
    ...
    return StreamBuilder<List<ZegoUIKitUser>>(
      ...
      builder: (context, roomSnapshot) {
        ...
        Widget content = Stack(
          fit: StackFit.expand,
          children: [
            if (_resolvedHostUid == null && !widget.isHost)
              Positioned(
                top: top + 9,
                left: 12,
                child: TextButton.icon(
                  onPressed: _resolveHost,
                  icon: const Icon(Icons.refresh_rounded, color: Colors.white70, size: 17),
                  label: const Text('Connecting...', style: TextStyle(color: Colors.white70, fontSize: 11)),
                ),
              ),
                ...
                child: Column(
                  ...
                  children: List.generate(
                    ...
                    (_) => const Padding(
                      ...
                      child: Icon(Icons.favorite_rounded, color: Color(0xFFFF3CA7), size: 36),
                    ),
                  ),
                ),
              ),
  ...
  Widget _hostIdentity(String name, String photo, int level) {
    return _glass(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _avatar(url: photo, name: name, radius: 20),
          const SizedBox(width: 8),
          Flexible(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
          ...
          const SizedBox(width: 5),
          const Icon(Icons.shield_rounded, color: Color(0xFFFFD24A), size: 20),
        ],
      ),
    );
  }

  Widget _rankingStrip() {
    final uid = _resolvedHostUid;
    if (uid == null) return const SizedBox.shrink();
          ...
          child: Row(
            ...
            children: [
              ...
              ...List.generate(docs.length, (index) {
                ...
                final sent = d['diamondsSent'] is num ? (d['diamondsSent'] as num).toInt() : 0;
                return _rankDataAvatar(index + 1, name, photo, level, sent);
              }),
            ],
          ),
        );
  ...
  Widget _rankDataAvatar(int rank, String name, String photo, int level, int sent) {
    final ring = rank == 1
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Stack(
            ...
            children: [
              Container(
                ...
                child: _avatar(url: photo, name: name, radius: 21),
              ),
              Positioned(
                bottom: -5,
                left: 13,
                child: Container(
                  ...
                  height: 18,
                  alignment: Alignment.center,
                  decoration: BoxDecoration(color: const Color(0xFF17131F), shape: BoxShape.circle, border: Border.all(color: ring)),
                  child: Text('$rank', style: TextStyle(color: ring, fontSize: 9, fontWeight: FontWeight.w900)),
                ),
              ),
            ],
          ),
          ...
          if (sent > 0)
            Text(_shortNumber(sent), style: const TextStyle(color: Colors.white70, fontSize: 7, fontWeight: FontWeight.w700)),
        ],
      ),
    );
  }
  ...
  Widget _diamondPill(int value) {
    return _glass(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.diamond_rounded, color: Color(0xFFC85CFF), size: 20),
          const SizedBox(width: 7),
          Text(_shortNumber(value), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 14)),
          const SizedBox(width: 6),
      ...
      builder: (context, snapshot) {
        ...
        return _glass(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Icon(Icons.local_fire_department_rounded, color: Color(0xFFFF7B3D), size: 18),
              const SizedBox(width: 5),
              const Text('Top 1', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12)),
              const SizedBox(width: 7),
              const Icon(Icons.diamond_rounded, color: Color(0xFFC85CFF), size: 15),
              const SizedBox(width: 3),
              Text(_shortNumber(value), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12)),
            ],
          ),
        );
      },
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 2),
                    Text(u.name.isEmpty ? 'Guest' : u.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800)),
                  ],
                ),
              ),
            )),
  ...
  Widget _guestSlot(BuildContext context, {bool compact = false}) {
    return InkWell(
      ...
      borderRadius: BorderRadius.circular(18),
      child: Container(
        width: compact ? 74 : 106,
        height: compact ? 74 : 106,
        decoration: BoxDecoration(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.add_rounded, color: Colors.white, size: compact ? 32 : 45),
            const SizedBox(height: 2),
            Text('Guest', style: TextStyle(color: Colors.white, fontSize: compact ? 11 : 15, fontWeight: FontWeight.w700)),
          ],
        ),
      ),
    );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(onPressed: _sendHeart, icon: const Icon(Icons.emoji_emotions_outlined, color: Colors.white, size: 21)),
        ],
      ),
    );
  }
  ...
  Widget _bottomControls() {
    return Row(
      ...
      children: [
        ...
        if (widget.isHost)
          _bottomButton(cameraOn ? Icons.videocam_rounded : Icons.videocam_off_rounded, 'Camera', _toggleCamera, active: cameraOn),
  ...
  Widget _glass({required Widget child, EdgeInsetsGeometry? padding, double radius = 18}) {
    return Container(
      padding: padding ?? const EdgeInsets.all(10),
      decoration: BoxDecoration(
        color: const Color(0xC915151A),
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            gradient: const LinearGradient(colors: [Color(0xFF8E35FF), Color(0xFF26126A)]),
            border: Border.all(color: const Color(0xFFCC72FF), width: 2),
            boxShadow: const [BoxShadow(color: Color(0xAA9C4DFF), blurRadius: 18)],
          ),
          child: const Icon(Icons.shield_rounded, color: Color(0xFFD7A8FF), size: 34),
        ),
        const SizedBox(height: 2),
        const Text('GUARDIAN', style: TextStyle(color: Color(0xFFD5A0FF), fontSize: 9, fontWeight: FontWeight.w900)),
      ],
    );
  }
  ...
  Widget _chatBubble(String name, String message) {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xA815151A), borderRadius: BorderRadius.circular(16)),
      child: RichText(
        text: TextSpan(children: [
          TextSpan(text: '${name.isEmpty ? 'User' : name}: ', style: const TextStyle(color: Color(0xFFD7B2FF), fontSize: 11, fontWeight: FontWeight.w900)),
          TextSpan(text: message, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600)),
        ]),
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Container(
            width: 52,
            height: 52,
            decoration: BoxDecoration(color: const Color(0xCC15151A), shape: BoxShape.circle, border: Border.all(color: active ? Colors.white24 : const Color(0xFFFF5C9D))),
            child: Icon(icon, color: active ? Colors.white : const Color(0xFFFF5C9D), size: 24),
          ),
          const SizedBox(height: 2),
      ...
      child: Column(
        ...
        children: [
          Container(
            width: 54,
            height: 54,
            decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF9D2BFF), Color(0xFFFF3AA8)]), border: Border.all(color: Colors.white54)),
            child: const Icon(Icons.card_giftcard_rounded, color: Colors.white, size: 27),
          ),
          const SizedBox(height: 2),
  ...
  Future<void> _sendGift(_GiftOption gift) async {
    ...
    try {
      await FirebaseFirestore.instance.runTransaction((tx) async {
        ...
        if (coins < gift.cost) {
          throw Exception('Not enough coins. Recharge coins first.');
        }
