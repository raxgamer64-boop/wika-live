...
abstract class _GameBaseState<T extends StatefulWidget> extends State<T> {
  ...
  Future<bool> start(int stake, String gameType) async {
    ...
    if (stake <= 0) {
      if (mounted) {
        setState(() => result = 'Invalid game entry.');
      }
      return false;
    }
    ...
    final ok = await _spendGameCoins(
      stake: stake,
      gameType: gameType,
    );
...
class GameCenterPage extends StatelessWidget {
  ...
  Widget _gameNote() => Container(
        ...
        child: const Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.info_outline_rounded, color: Color(0xFFFFD76A), size: 20), SizedBox(width: 10), Expanded(child: Text('WikaLive games use virtual Coins. Rewards stay inside the app and can be used for supported in-app gifting and features; they are not cash.', style: TextStyle(color: Colors.white60, height: 1.35, fontSize: 12)))]),
      );
}
