...
class _LiveScreenState extends State<LiveScreen> {
  ...
  Stream<List<QueryDocumentSnapshot<Map<String,dynamic>>>> _hosts()=>_base().snapshots().map((s){
    ...
    if(section=='New Host'){final cutoff=DateTime.now().subtract(const Duration(days:7));list.removeWhere((d){final v=d.data()['createdAt'];return v is! Timestamp||_date(v).isBefore(cutoff);});}
    return list;
  });
  Future<Set<String>> _following()async{final uid=FirebaseAuth.instance.currentUser?.uid;if(uid==null)return{};final q=await FirebaseFirestore.instance.collection('users').doc(uid).collection('following').get();return q.docs.map((e)=>e.id).toSet();}
  @override Widget build(BuildContext context){
    return Container(color:const Color(0xFF09070F),child:SafeArea(child:CustomScrollView(slivers:[
      SliverToBoxAdapter(child:Padding(padding:const EdgeInsets.fromLTRB(18,14,18,8),child:Row(children:[const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('WikaLive',style:TextStyle(color:Colors.white,fontSize:28,fontWeight:FontWeight.w900)),SizedBox(height:2),Text('Discover people who are live now',style:TextStyle(color:Colors.white54,fontSize:11,fontWeight:FontWeight.w600))])),_glass(Icons.search_rounded,()=>openPage(context,const UserSearchPage())),const SizedBox(width:8),_glass(Icons.leaderboard_rounded,()=>openPage(context,const RankingPage()))]))),
      SliverToBoxAdapter(child:SizedBox(height:44,child:ListView.separated(padding:const EdgeInsets.symmetric(horizontal:18),scrollDirection:Axis.horizontal,itemCount:sections.length,separatorBuilder:(_,__)=>const SizedBox(width:8),itemBuilder:(c,i){final x=sections[i];final active=section==x;return InkWell(onTap:(){if(x=='Country'){openPage(context,const CountryLivePage());return;}setState(()=>section=x);},borderRadius:BorderRadius.circular(22),child:AnimatedContainer(duration:const Duration(milliseconds:180),padding:const EdgeInsets.symmetric(horizontal:16,vertical:11),decoration:BoxDecoration(gradient:active?const LinearGradient(colors:[Color(0xFF9B55E8),Color(0xFFE052A5)]):null,color:active?null:const Color(0xFF15111D),borderRadius:BorderRadius.circular(22),border:Border.all(color:active?Colors.transparent:const Color(0x18FFFFFF))),child:Text(x,style:TextStyle(color:active?Colors.white:Colors.white60,fontSize:12,fontWeight:FontWeight.w800))));}))),
      SliverToBoxAdapter(child:_hero(context)),
      SliverToBoxAdapter(child:Padding(padding:const EdgeInsets.fromLTRB(18,4,18,12),child:Row(children:[const Text('Live now',style:TextStyle(color:Colors.white,fontSize:19,fontWeight:FontWeight.w900)),const Spacer(),Text(section,style:const TextStyle(color:Colors.white38,fontSize:11,fontWeight:FontWeight.w700))]))),
      StreamBuilder<List<QueryDocumentSnapshot<Map<String,dynamic>>>>(stream:_hosts(),builder:(context,snap){if(snap.connectionState==ConnectionState.waiting)return const SliverFillRemaining(child:Center(child:CircularProgressIndicator()));var docs=snap.data??[];if(section=='Follow')return SliverToBoxAdapter(child:FutureBuilder<Set<String>>(future:_following(),builder:(c,f)=>_grid(c,docs.where((d)=>(f.data??{}).contains(d.id)).toList(),'No followed host is live right now.')));if(section=='Nearby')return SliverToBoxAdapter(child:_nearby(context,docs));return SliverToBoxAdapter(child:_grid(context,docs,'No live hosts found.'));})
    ])));
  }
  Widget _glass(IconData icon,VoidCallback tap)=>InkWell(onTap:tap,borderRadius:BorderRadius.circular(18),child:Container(width:44,height:44,decoration:BoxDecoration(color:const Color(0xFF15111D),borderRadius:BorderRadius.circular(18),border:Border.all(color:const Color(0x18FFFFFF))),child:Icon(icon,color:Colors.white,size:21)));
  Widget _hero(BuildContext c)=>Container(margin:const EdgeInsets.fromLTRB(18,10,18,18),height:150,padding:const EdgeInsets.all(20),decoration:BoxDecoration(gradient:const LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[Color(0xFF28133E),Color(0xFF7B2F72),Color(0xFFE052A5)]),borderRadius:BorderRadius.circular(28),boxShadow:const[BoxShadow(color:Color(0x552D0C43),blurRadius:28,offset:Offset(0,12))]),child:Stack(children:[const Positioned(right:-5,top:-20,child:Icon(Icons.auto_awesome,size:120,color:Color(0x22FFFFFF))),Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Container(padding:const EdgeInsets.symmetric(horizontal:9,vertical:5),decoration:BoxDecoration(color:const Color(0x33FFFFFF),borderRadius:BorderRadius.circular(9)),child:const Text('LIVE • REAL TIME',style:TextStyle(color:Color(0xFFFFD76A),fontSize:10,fontWeight:FontWeight.w900,letterSpacing:1))),const Spacer(),const Text('Your next
favorite host is live.',style:TextStyle(color:Colors.white,fontSize:22,fontWeight:FontWeight.w900,height:1.02)),const SizedBox(height:7),const Text('Watch • Chat • Follow • Gift',style:TextStyle(color:Colors.white70,fontSize:11,fontWeight:FontWeight.w700))]),Positioned(right:4,bottom:4,child:IconButton(onPressed:()=>openPage(c,const HostPage()),icon:const Icon(Icons.add_circle,color:Colors.white,size:34))))]));
  Widget _grid(BuildContext c,List<QueryDocumentSnapshot<Map<String,dynamic>>> docs,String empty)=>docs.isEmpty?Container(height:300,alignment:Alignment.center,child:Text(empty,style:const TextStyle(color:Colors.white54,fontWeight:FontWeight.w700))):GridView.builder(shrinkWrap:true,physics:const NeverScrollableScrollPhysics(),padding:const EdgeInsets.fromLTRB(18,0,18,24),itemCount:docs.length,gridDelegate:const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:2,crossAxisSpacing:12,mainAxisSpacing:12,childAspectRatio:.68),itemBuilder:(c,i){final x=docs[i].data();final raw=x['photos'];final photos=raw is List?raw:[];final photo=photos.isNotEmpty?photos.first.toString():(x['photoUrl']??'').toString();final name=(x['name']??'Host').toString();final viewers=x['viewers']??0;return InkWell(onTap:()=>openPage(c,LiveRoomPage(host:name,viewers:'$viewers viewers')),borderRadius:BorderRadius.circular(24),child:ClipRRect(borderRadius:BorderRadius.circular(24),child:Stack(fit:StackFit.expand,children:[photo.isEmpty?Container(color:const Color(0xFF21182A),child:const Icon(Icons.person_rounded,size:70,color:Color(0xFFB46BE0))):Image.network(photo,fit:BoxFit.cover,errorBuilder:(_,__,___)=>Container(color:const Color(0xFF21182A),child:const Icon(Icons.person_rounded,size:70,color:Color(0xFFB46BE0)))),const DecoratedBox(decoration:BoxDecoration(gradient:LinearGradient(begin:Alignment.topCenter,end:Alignment.center,colors:[Color(0x66000000),Colors.transparent]))),const DecoratedBox(decoration:BoxDecoration(gradient:LinearGradient(begin:Alignment.center,end:Alignment.bottomCenter,colors:[Colors.transparent,Color(0xF5090610)]))),Positioned(top:10,left:10,child:_badge('LIVE')),Positioned(top:10,right:10,child:Container(padding:const EdgeInsets.symmetric(horizontal:8,vertical:5),decoration:BoxDecoration(color:Colors.black45,borderRadius:BorderRadius.circular(12)),child:Text('${x['countryFlag']??'🌍'}',style:const TextStyle(fontSize:13)))),Positioned(left:12,right:12,bottom:12,child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(name,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(color:Colors.white,fontSize:16,fontWeight:FontWeight.w900)),const SizedBox(height:5),Row(children:[const Icon(Icons.remove_red_eye_outlined,color:Colors.white60,size:14),const SizedBox(width:4),Text('$viewers watching',style:const TextStyle(color:Colors.white70,fontSize:10,fontWeight:FontWeight.w700)),const Spacer(),const Icon(Icons.card_giftcard_rounded,color:Color(0xFFFFD76A),size:17)])]))])));});
  Widget _badge(String t)=>Container(padding:const EdgeInsets.symmetric(horizontal:8,vertical:5),decoration:BoxDecoration(color:const Color(0xFFE44787),borderRadius:BorderRadius.circular(9)),child:Text(t,style:const TextStyle(color:Colors.white,fontSize:9,fontWeight:FontWeight.w900)));
  Widget _nearby(BuildContext c,List<QueryDocumentSnapshot<Map<String,dynamic>>> docs){final uid=FirebaseAuth.instance.currentUser?.uid;if(uid==null)return _grid(c,[],'Please login again.');return FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(future:FirebaseFirestore.instance.collection('users').doc(uid).get(),builder:(c,s){final m=s.data?.data()??{};final lat=_num(m['latitude']);final lon=_num(m['longitude']);if(lat==0&&lon==0)return const SizedBox(height:250,child:Center(child:Text('Nearby needs saved location.',style:TextStyle(color:Colors.white54))));final near=docs.where((d){final x=d.data();final a=_num(x['latitude']);final b=_num(x['longitude']);final dx=(a-lat).abs()*111;final dy=(b-lon).abs()*111;return a!=0&&b!=0&&(dx*dx+dy*dy)<2500;}).toList();return _grid(c,near,'No live host found within 50 km.');});}
}
...
class PartyScreen extends StatelessWidget {
  ...
  static const rooms=[['Midnight Vibes','Music • Chill','31'],['SINGH Agency','Talk • Meet','24'],['Suman’s Room','Friends • Chat','18'],['Late Night Cafe','Open Mic','12']];
  @override Widget build(BuildContext context)=>Container(color:const Color(0xFF09070F),child:SafeArea(child:ListView(padding:const EdgeInsets.fromLTRB(18,14,18,28),children:[
    Row(children:[const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Party',style:TextStyle(color:Colors.white,fontSize:28,fontWeight:FontWeight.w900)),Text('Voice rooms & social nights',style:TextStyle(color:Colors.white45,fontSize:11,fontWeight:FontWeight.w600))])),_a(Icons.search_rounded,()=>openPage(context,const FeaturePage(title:'Search Party'))),const SizedBox(width:8),_a(Icons.add_rounded,()=>openPage(context,const CreatePartyPage()))]),
    const SizedBox(height:16),Container(height:170,padding:const EdgeInsets.all(20),decoration:BoxDecoration(gradient:const LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[Color(0xFF21102F),Color(0xFF63306F),Color(0xFFB8498E)]),borderRadius:BorderRadius.circular(28)),child:Stack(children:[const Positioned(right:-8,bottom:-22,child:Icon(Icons.graphic_eq_rounded,size:150,color:Color(0x1FFFFFFF))),Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('PARTY WORLD',style:TextStyle(color:Color(0xFFFFD76A),fontSize:10,fontWeight:FontWeight.w900,letterSpacing:1.4)),const Spacer(),const Text('Find your people.',style:TextStyle(color:Colors.white,fontSize:25,fontWeight:FontWeight.w900)),const SizedBox(height:5),const Text('Talk • Sing • Laugh • Connect',style:TextStyle(color:Colors.white70,fontSize:12,fontWeight:FontWeight.w700))]),Positioned(right:2,bottom:2,child:FilledButton(onPressed:()=>openPage(context,const CreatePartyPage()),style:FilledButton.styleFrom(backgroundColor:Colors.white,foregroundColor:const Color(0xFF5C2B69)),child:const Text('Create room'))))])),
    const SizedBox(height:16),SizedBox(height:40,child:ListView.separated(scrollDirection:Axis.horizontal,itemCount:5,separatorBuilder:(_,__)=>const SizedBox(width:8),itemBuilder:(c,i){final x=['Trending','Music','Talk','PK','New'];return Container(padding:const EdgeInsets.symmetric(horizontal:15,vertical:10),decoration:BoxDecoration(color:i==0?const Color(0xFF9B55E8):const Color(0xFF15111D),borderRadius:BorderRadius.circular(20)),child:Text(x,style:TextStyle(color:i==0?Colors.white:Colors.white54,fontSize:11,fontWeight:FontWeight.w800)));})),
    const SizedBox(height:16),const Text('Rooms you may like',style:TextStyle(color:Colors.white,fontSize:19,fontWeight:FontWeight.w900)),const SizedBox(height:10),...rooms.map((r)=>_room(context,r)),
  ])));
  Widget _a(IconData i,VoidCallback t)=>InkWell(onTap:t,borderRadius:BorderRadius.circular(18),child:Container(width:44,height:44,decoration:BoxDecoration(color:const Color(0xFF15111D),borderRadius:BorderRadius.circular(18),border:Border.all(color:const Color(0x18FFFFFF))),child:Icon(i,color:Colors.white)));
  Widget _room(BuildContext c,List<String> r)=>InkWell(onTap:()=>openPage(c,PartyRoomPage(title:r[0])),borderRadius:BorderRadius.circular(20),child:Container(margin:const EdgeInsets.only(bottom:10),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:const Color(0xFF120E18),borderRadius:BorderRadius.circular(20),border:Border.all(color:const Color(0x14FFFFFF))),child:Row(children:[Container(width:70,height:70,decoration:BoxDecoration(gradient:const LinearGradient(colors:[Color(0xFFE65CA7),Color(0xFF7540D8)]),borderRadius:BorderRadius.circular(18)),child:const Icon(Icons.mic_rounded,color:Colors.white,size:34)),const SizedBox(width:13),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(r[0],style:const TextStyle(color:Colors.white,fontSize:16,fontWeight:FontWeight.w900)),const SizedBox(height:5),Text(r[1],style:const TextStyle(color:Colors.white45,fontSize:11,fontWeight:FontWeight.w700)),const SizedBox(height:10),Row(children:[const Icon(Icons.circle,color:Color(0xFF55E68A),size:8),const SizedBox(width:5),Text('${r[2]} online',style:const TextStyle(color:Colors.white60,fontSize:10,fontWeight:FontWeight.w700)),const Spacer(),const Icon(Icons.chevron_right_rounded,color:Colors.white38)]))]))])));
}
...
class MeScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){final u=FirebaseAuth.instance.currentUser;final uid=u?.uid;if(uid==null)return const Center(child:Text('Please login again',style:TextStyle(color:Colors.white)));return StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('users').doc(uid).snapshots(),builder:(context,s){final d=s.data?.data()??{};final name=(d['name']??u?.displayName??'WikaLive User').toString();final id=(d['id']??uid.substring(0,8)).toString();final raw=d['photos'];final photos=raw is List?raw:[];final photo=photos.isNotEmpty?photos.first.toString():(d['photoUrl']??'').toString();return Container(color:const Color(0xFF09070F),child:SafeArea(child:ListView(padding:const EdgeInsets.fromLTRB(18,14,18,28),children:[
    Row(children:[const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Me',style:TextStyle(color:Colors.white,fontSize:28,fontWeight:FontWeight.w900)),Text('Your WikaLive space',style:TextStyle(color:Colors.white45,fontSize:11,fontWeight:FontWeight.w600))])),IconButton(onPressed:()=>openPage(context,const SettingsPage()),icon:const Icon(Icons.settings_rounded,color:Colors.white70))]),
...
class ProfilePage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){final u=FirebaseAuth.instance.currentUser;if(u==null)return const Scaffold(body:Center(child:Text('Please login again')));return StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('users').doc(u.uid).snapshots(),builder:(context,s){final d=s.data?.data()??{};final raw=d['photos'];final photos=raw is List?raw.map((e)=>e.toString()).where((e)=>e.isNotEmpty).take(5).toList():<String>[];if(photos.isEmpty&&(d['photoUrl']??'').toString().isNotEmpty)photos.add(d['photoUrl'].toString());final name=(d['name']??u.displayName??'WikaLive User').toString();return Scaffold(backgroundColor:const Color(0xFF09070F),appBar:AppBar(backgroundColor:const Color(0xFF09070F),foregroundColor:Colors.white,title:const Text('Profile',style:TextStyle(fontWeight:FontWeight.w900)),actions:[IconButton(onPressed:()=>openPage(context,const EditProfilePage()),icon:const Icon(Icons.edit_rounded))]),body:ListView(padding:const EdgeInsets.only(bottom:28),children:[
    SizedBox(height:360,child:Stack(fit:StackFit.expand,children:[photos.isEmpty?Container(color:const Color(0xFF1B1422),child:const Icon(Icons.person_rounded,size:110,color:Color(0xFFB46BE0))):PageView.builder(itemCount:photos.length,itemBuilder:(c,i)=>Image.network(photos[i],fit:BoxFit.cover,errorBuilder:(_,__,___)=>Container(color:const Color(0xFF1B1422),child:const Icon(Icons.person_rounded,size:110,color:Color(0xFFB46BE0)))),const DecoratedBox(decoration:BoxDecoration(gradient:LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[Colors.transparent,Color(0xFF09070F)]))),Positioned(left:18,right:18,bottom:18,child:Row(children:[Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(name,style:const TextStyle(color:Colors.white,fontSize:27,fontWeight:FontWeight.w900)),const SizedBox(height:5),Text('ID: ${(d['id']??u.uid.substring(0,8))}',style:const TextStyle(color:Colors.white60,fontSize:10,fontWeight:FontWeight.w700))])),Container(padding:const EdgeInsets.symmetric(horizontal:12,vertical:8),decoration:BoxDecoration(color:const Color(0x55FFFFFF),borderRadius:BorderRadius.circular(16)),child:Text('${photos.length}/5 photos',style:const TextStyle(color:Colors.white,fontSize:10,fontWeight:FontWeight.w800)))])])),
    Padding(padding:const EdgeInsets.fromLTRB(18,4,18,0),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text((d['bio']??'').toString(),style:const TextStyle(color:Colors.white70,fontSize:13,height:1.4)),const SizedBox(height:14),Wrap(spacing:7,runSpacing:7,children:[_tag('🌍 ${d['country']??'India'}'),_tag('🎙 ${d['language']??'English'}'),if('${d['tag']??''}'.isNotEmpty)_tag('#${d['tag']}')]),const SizedBox(height:18),Row(children:[Expanded(child:FilledButton.icon(onPressed:()=>openPage(context,const FollowingPage()),icon:const Icon(Icons.person_add_alt_1),label:const Text('Following'))),const SizedBox(width:10),Expanded(child:OutlinedButton.icon(onPressed:()=>openPage(context,const FollowersPage()),icon:const Icon(Icons.people_alt_outlined),label:const Text('Followers')))]),const SizedBox(height:18),const Text('About',style:TextStyle(color:Colors.white,fontSize:18,fontWeight:FontWeight.w900)),const SizedBox(height:10),_line('Gender',d['gender']),_line('Birthday',d['birthday']),_line('Language',d['language']),_line('Country',d['country'])]) )
  ]);});}
  Widget _tag(String t)=>Container(padding:const EdgeInsets.symmetric(horizontal:11,vertical:7),decoration:BoxDecoration(color:const Color(0xFF17111F),borderRadius:BorderRadius.circular(13),border:Border.all(color:const Color(0x18FFFFFF))),child:Text(t,style:const TextStyle(color:Colors.white70,fontSize:10,fontWeight:FontWeight.w800)));
  Widget _line(String a,d)=>d==null||'$d'.isEmpty?const SizedBox.shrink():Padding(padding:const EdgeInsets.only(bottom:10),child:Row(children:[Text(a,style:const TextStyle(color:Colors.white38,fontSize:11,fontWeight:FontWeight.w700)),const Spacer(),Text('$d',style:const TextStyle(color:Colors.white70,fontSize:11,fontWeight:FontWeight.w800))]));
}
